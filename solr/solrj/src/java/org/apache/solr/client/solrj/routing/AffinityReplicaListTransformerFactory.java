 ...
 *
 * <p>Default names of params that contain the values by which routing is determined may be
 * configured at the time of {@link AffinityReplicaListTransformerFactory} construction, and may be
 * overridden by the config spec passed to {@link #getInstance(String, SolrParams,
 * ReplicaListTransformerFactory)}
 *
 * <p>If no defaultHashParam name is specified at time of factory construction, the routing dividend
 * will be derived by hashing the {@link String} value of the {@link CommonParams#Q} param.
 */
public class AffinityReplicaListTransformerFactory implements ReplicaListTransformerFactory {
  ...
  public AffinityReplicaListTransformerFactory(
      String defaultDividendParam, String defaultHashParam) {
  ...
  public AffinityReplicaListTransformerFactory(NamedList<?> c) {
    this(
        (String) c.get(ShardParams.ROUTING_DIVIDEND),
        translateHashParam((String) c.get(ShardParams.ROUTING_HASH)));
  }

  /**
   * Null arg indicates no configuration, which should be translated to the default value {@link
  ...
  @Override
  public ReplicaListTransformer getInstance(
