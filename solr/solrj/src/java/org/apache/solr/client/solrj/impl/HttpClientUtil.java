...
public class HttpClientUtil {
  ...
  private static SolrHttpClientContextBuilder httpClientRequestContextBuilder =
      new SolrHttpClientContextBuilder();
  ...
  static {
    ...
    String socketFactoryRegistryProviderClassName =
        System.getProperty(SYS_PROP_SOCKET_FACTORY_REGISTRY_PROVIDER);
   ...
   *
   * @param params http client configuration, if null a client with default configuration (no
   *     additional configuration) is created.
   */
  ...
  public static CloseableHttpClient createClient(
      SolrParams params, PoolingHttpClientConnectionManager cm) {
    if (params == null) {
  ...
  private static HttpClientBuilder setupBuilder(HttpClientBuilder builder, SolrParams config) {
    ...
    Builder requestConfigBuilder =
        RequestConfig.custom()
            ...
            .setConnectTimeout(
                config.getInt(HttpClientUtil.PROP_CONNECTION_TIMEOUT, DEFAULT_CONNECT_TIMEOUT))
    ...
    if (config.getBool(HttpClientUtil.PROP_USE_RETRY, true)) {
      retBuilder =
          retBuilder.setRetryHandler(
              new SolrHttpRequestRetryHandler(Integer.getInteger("solr.httpclient.retries", 3)));
  ...
  protected static class GzipDecompressingEntity extends HttpEntityWrapper {
    ...
    /**
     * Return a InputStream of uncompressed data. If there is an issue with the compression of the
     * data, a null InputStream will be returned, and the underlying compressed InputStream will be
     * closed.
     *
     * <p>The same input stream will be returned if the underlying entity is not repeatable. If the
     * underlying entity is repeatable, then a new input stream will be created.
     */
    @Override
  ...
  public static void setHttpClientRequestContextBuilder(
      SolrHttpClientContextBuilder httpClientContextBuilder) {
