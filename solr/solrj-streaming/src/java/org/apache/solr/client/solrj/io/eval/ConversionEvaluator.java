...
public class ConversionEvaluator extends RecursiveNumericEvaluator implements OneValueWorker {
  ...
  public ConversionEvaluator(StreamExpression expression, StreamFactory factory)
      throws IOException {
    ...
    if (3 != containedEvaluators.size()) {
      throw new IOException(
          String.format(
              Locale.ROOT,
              "Invalid expression %s - expecting exactly 3 parameters but found %d",
    ...
    if (containedEvaluators.subList(0, 2).stream()
        .anyMatch(
            item ->
                !(item instanceof RawValueEvaluator) && !(item instanceof FieldValueEvaluator))) {
      throw new IOException(
          String.format(
              Locale.ROOT,
              "Invalid expression %s - first two parameters must be strings",
              super.toExpression(constructingFactory)));
    }

    String fromString =
        containedEvaluators.get(0).toExpression(factory).toString().toUpperCase(Locale.ROOT);
    String toString =
        containedEvaluators.get(1).toExpression(factory).toString().toUpperCase(Locale.ROOT);
