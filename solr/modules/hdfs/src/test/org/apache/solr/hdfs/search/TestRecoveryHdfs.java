...
public class TestRecoveryHdfs extends SolrTestCaseJ4 {
  ...
  public void testCorruptLog() throws Exception {
    try {
      ...
      updateJ(
          jsonAdd(sdoc("id", "G4", "_version_", "104")), params(DISTRIB_UPDATE_PARAM, FROM_LEADER));
      updateJ(
          jsonAdd(sdoc("id", "G5", "_version_", "105")), params(DISTRIB_UPDATE_PARAM, FROM_LEADER));
      updateJ(
          jsonAdd(sdoc("id", "G6", "_version_", "106")), params(DISTRIB_UPDATE_PARAM, FROM_LEADER));
  ...
  public void testRecoveryMultipleLogs() throws Exception {
    try {
      ...
      try (FSDataInputStream dis = fs.open(new Path(logDir, files[files.length - 1]))) {
        ...
        // Now make a newer log file with just the IDs changed.  NOTE: this may not work if log
        // format changes too much!
        findReplace(
