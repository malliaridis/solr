 ...
 * framework. This plugin supports following features - integration with authentication mechanisms
 * (e.g. kerberos) - Delegation token support - Proxy users (or secure impersonation) support
 *
 * <p>This plugin enables defining configuration parameters required by the underlying Hadoop
...
public class HadoopAuthPlugin extends AuthenticationPlugin {
  ...
  protected FilterConfig getInitFilterConfig(Map<String, Object> pluginConfig) {
    ...
    if (delegationTokenEnabled) {
      ...
      params.putIfAbsent(
          "delegation-token.token-kind", KerberosPlugin.DELEGATION_TOKEN_TYPE_DEFAULT);
    }
