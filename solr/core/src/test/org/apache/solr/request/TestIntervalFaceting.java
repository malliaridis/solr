...
public class TestIntervalFaceting extends SolrTestCaseJ4 {
  ...
  public void testMultiValueFields() {
    ...
    assertIntervalQuery(
        "test_ss_dv", "[*,*]", "2", "[*,cat)", "1", "[cat,dog)", "2", "[dog,*)", "1");
  }
  ...
  public void testBasic() {
    ...
    assertQEx(
        ...
        req(
            ...
            "facet.interval",
            "test_s_dv",
            "f.test_s_dv.facet.interval.set",
            "bird,bird]"),
        SolrException.ErrorCode.BAD_REQUEST);
    assertQEx(
        "only separator is escaped",
        req(
            "fl",
            "test_s_dv",
            "q",
  ...
  public void testMultipleFields() {
    ...
    assertQ(
        req(
            ...
            "f.test_s_dv.facet.interval.set",
            "[cat,dog]",
            "f.test_l_dv.facet.interval.set",
            "[3,6]",
            "f.test_l_dv.facet.interval.set",
            "[5,9]"),
  ...
  public void testWithFieldCache() {
    ...
    assertQ(
        req(
            ...
            "facet.interval",
            "test_l",
            "f.test_s.facet.interval.set",
            "[cat,dog]",
            "f.test_l.facet.interval.set",
            "[3,6]",
            "f.test_l.facet.interval.set",
            "[5,9]"),
  ...
  public void testRandom() throws Exception {
    ...
    for (int i = 0; i < docs; i++) {
      ...
      docFields[1] =
          String.valueOf(
              i * (random().nextBoolean() ? 1 : -1)); // in the queries we do positive and negative
      docFields[2] = "test_s";
  ...
  private void doTestQuery(int cardinality, String[] fields) throws Exception {
    ...
    try (SolrQueryRequest req = req(params)) {
      ...
      NamedList<Object> facetQueries =
          (NamedList<Object>)
              ((NamedList<Object>) rsp.getValues().get("facet_counts")).get("facet_queries");
      NamedList<Object> facetIntervals =
          (NamedList<Object>)
              ((NamedList<Object>)
                      ((NamedList<Object>) rsp.getValues().get("facet_counts"))
                          .get("facet_intervals"))
                  .get(field);
      assertEquals(
          "Responses don't have the same number of facets: \n"
              + facetQueries
              + "\n"
              + facetIntervals,
          facetQueries.size(),
   ...
   * The range could have values greater than "max", for example
   * [Integer/Long/Float/Double].[MIN/MAX_VALUE,POSITIVE/NEGATIVE_INFINITY] If the fieldName is
   * "test_s_dv" or "test_ss_dv" (the two fields used for Strings), the comparison will be done
   * alphabetically If the field is a Date, a date range will be returned The range could also
   * contain "*" as beginning and/or end of the range
   */
  ...
  public void testParse() throws SyntaxError {
    assertInterval(
        ...
        "(0,2)",
        new long[] {1},
        new long[] {0, -1, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {2, 3, Integer.MAX_VALUE, Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
        "(0,2]",
        new long[] {1, 2},
        new long[] {0, -1, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {3, Integer.MAX_VALUE, Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
        "[0,2]",
        new long[] {0, 1, 2},
        new long[] {-1, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {3, Integer.MAX_VALUE, Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
        "[0,2)",
    ...
    assertInterval(
        "test_l_dv",
        "(0,*)",
        new long[] {1, 2, 3, Integer.MAX_VALUE, Long.MAX_VALUE},
        new long[] {-1, 0, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {});
    assertInterval(
        "test_l_dv",
        "(*,2)",
        new long[] {-1, Integer.MIN_VALUE, Long.MIN_VALUE, 0, 1},
        new long[] {},
        new long[] {2, 3, Integer.MAX_VALUE, Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
        "(*,*)",
        new long[] {
          -1, Integer.MIN_VALUE, Long.MIN_VALUE, 0, 1, 2, 3, Integer.MAX_VALUE, Long.MAX_VALUE
        },
        new long[] {},
        new long[] {});
    ...
    assertInterval(
        "test_l_dv",
        "[0,*]",
        new long[] {0, 1, 2, 3, Integer.MAX_VALUE, Long.MAX_VALUE},
        new long[] {-1, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {});
    assertInterval(
        "test_l_dv",
        "[*,2]",
        new long[] {-1, Integer.MIN_VALUE, Long.MIN_VALUE, 0, 1, 2},
        new long[] {},
        new long[] {3, Integer.MAX_VALUE, Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
    ...
    assertInterval(
        ...
        "(2,2)",
        new long[] {},
        new long[] {2, 1, 0, -1, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {3, Integer.MAX_VALUE, Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
        "(0,0)",
    ...
    assertInterval(
        "test_l_dv",
        "(0," + Long.MAX_VALUE + ")",
        new long[] {1, 2, 3, Integer.MAX_VALUE},
        new long[] {0, -1, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
        "(" + Long.MIN_VALUE + ",0)",
        new long[] {-1, Integer.MIN_VALUE},
        new long[] {Long.MIN_VALUE},
        new long[] {1, 2, Integer.MAX_VALUE, Long.MAX_VALUE});
    assertInterval(
        "test_l_dv",
        "[" + Long.MIN_VALUE + ",0)",
        new long[] {-1, Integer.MIN_VALUE, Long.MIN_VALUE},
    ...
    assertInterval(
        ...
        new long[] {-1, Integer.MIN_VALUE, Long.MIN_VALUE, 1, 2, Integer.MAX_VALUE, Long.MAX_VALUE},
        new long[] {},
        new long[] {});
    assertInterval(
        "test_l_dv",
        "(" + Long.MIN_VALUE + "," + Long.MAX_VALUE + ")",
        new long[] {-1, Integer.MIN_VALUE, 1, 2, Integer.MAX_VALUE},
        new long[] {Long.MIN_VALUE},
        new long[] {Long.MAX_VALUE});
    ...
    assertInterval(
        ...
        "[-1,1]",
        new long[] {-1, 0, 1},
        new long[] {-2, Integer.MIN_VALUE, Long.MIN_VALUE},
        new long[] {2, 3, Integer.MAX_VALUE, Long.MAX_VALUE});
    ...
    assertQ(
        ...
        "count(//lst[@name='facet_intervals']/lst)=2",
        "//lst[@name='facet_intervals']/lst[@name='test_s_dv']/int[@name='[1,2]'][.=0]",
        "count(//lst[@name='facet_intervals']/lst[@name='test_l_dv']/int)=2", // interval [1,2] not
        // present
        "//lst[@name='facet_intervals']/lst[@name='test_l_dv']/int[@name='[3,4]'][.=2]",
    ...
    assertQ(
        req(
            ...
            "[cat,dog]"),
        "count(//lst[@name='facet_intervals']/lst)=2",
        "count(//lst[@name='facet_intervals']/lst[@name='test_s_dv']/int)=1", // only [cat,dog] in
        // test_s_dv
        "//lst[@name='facet_intervals']/lst[@name='test_s_dv']/int[@name='[cat,dog]'][.=5]",
  ...
  private void assertStringInterval(
    ...
    assertEquals(
        "Expected start " + expectedStart + " but found " + f.getType().toObject(f, interval.start),
        interval.start,
        new BytesRef(f.getType().toInternal(expectedStart)));

    assertEquals(
        "Expected end " + expectedEnd + " but found " + f.getType().toObject(f, interval.end),
        interval.end,
        new BytesRef(f.getType().toInternal(expectedEnd)));
  }
  ...
  private void assertBadInterval(String fieldName, String intervalStr, String errorMsg) {
    ...
    SyntaxError e =
        expectThrows(
            SyntaxError.class, () -> new FacetInterval(f, intervalStr, new ModifiableSolrParams()));
    assertTrue(
        "Unexpected error message for interval String: " + intervalStr + ": " + e.getMessage(),
        e.getMessage().contains(errorMsg));
  }

  private void assertInterval(
      String fieldName,
      String intervalStr,
      long[] included,
      long[] lowerThanStart,
    ...
    for (long l : included) {
      assertEquals(
          "Value " + l + " should be INCLUDED for interval" + interval,
          IntervalCompareResult.INCLUDED,
          interval.includes(l));
    }
  ...
  private void assertIntervalKey(
    ...
    assertEquals(
        "Expected key " + expectedKey + " but found " + interval.getKey(),
        expectedKey,
        interval.getKey());
  }
  ...
  public void testIntMultivaluedFields() {
    ...
    assertU(
        adoc(
            "id", "1", "test_is_dv", "0", "test_is_dv", "1", "test_is_dv", "2", "test_is_dv", "3"));
    assertU(
        adoc(
            "id", "2", "test_is_dv", "1", "test_is_dv", "2", "test_is_dv", "3", "test_is_dv", "4"));
    assertU(
        adoc(
            "id", "3", "test_is_dv", "2", "test_is_dv", "3", "test_is_dv", "4", "test_is_dv", "5"));
    assertU(
        adoc(
            "id", "4", "test_is_dv", "3", "test_is_dv", "4", "test_is_dv", "5", "test_is_dv", "6"));
    assertU(commit());
  ...
  private void assertIntervalQueriesNumeric(String field) {
    ...
    assertIntervalQuery(
        field, "[*,3)", "3", "[2,5)", "3", "[6,8)", "2", "[3,*]", "7", "[10,10]", "1", "[10,10]",
        "1", "[10,10]", "1");
  ...
  private void assertIntervalQueriesString(String field) {
    ...
    assertIntervalQuery(
        field, "[*,bird)", "2", "[bird,cat)", "1", "[cat,dog)", "2", "[dog,*]", "4");
  ...
  /**
   * Will run a match all query, and ask for interval facets in the specified field. The intervals
   * facet are indicated in the <code>intervals</code> parameter, followed by the expected count
   * result. For example: <code>
   * assertIntervalQuery("my_string_field", "[0,10]", "3", "(20,*), "12");</code>
   *
  ...
  private void assertIntervalQuery(
    ...
    for (int i = 0; i < intervals.length; i += 2) {
      tests[idx++] =
          "//lst[@name='facet_intervals']/lst[@name='"
              + field
              + "']/int[@name='"
              + intervals[i]
              + "'][.="
