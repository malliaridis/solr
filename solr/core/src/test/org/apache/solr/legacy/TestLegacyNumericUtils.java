...
public class TestLegacyNumericUtils extends SolrTestCase {
  ...
  public void testLongConversionAndOrdering() {
    ...
    for (long l = -100000L; l < 100000L; l++) {
      ...
      if (last != null) {
        ...
        assertTrue(
            "actual bigger than last (as String)",
            last.get().utf8ToString().compareTo(act.get().utf8ToString()) < 0);
      }
      // test is back and forward conversion works
      assertEquals(
          "forward and back conversion should generate same long",
          l,
          LegacyNumericUtils.prefixCodedToLong(act.get()));
      // next step
      last.copyBytes(act);
    }
  }
  ...
  public void testIntConversionAndOrdering() {
    ...
    for (int i = -100000; i < 100000; i++) {
      ...
      assertEquals(
          ...
          i,
          LegacyNumericUtils.prefixCodedToInt(act.get()));
      // next step
  ...
  private void assertLongRangeSplit(
      final long lower,
      final long upper,
      int precisionStep,
      final boolean useBitSet,
      final Iterable<Long> expectedBounds,
      final Iterable<Integer> expectedShifts) {
    ...
    final Iterator<Integer> neededShifts =
        (expectedShifts == null) ? null : expectedShifts.iterator();
  ...
  public void testLongExtremeValues() {
    ...
    assertLongRangeSplit(
        ...
        1,
        true,
        Arrays.asList(0xffffffffffffffffL, 0xffffffffffffffffL),
        Arrays.asList(0));
    assertLongRangeSplit(
        Long.MAX_VALUE,
        Long.MAX_VALUE,
    ...
    assertLongRangeSplit(
        ...
        4,
        true,
        Arrays.asList(0xfffffffffffffffL, 0xfffffffffffffffL),
        Arrays.asList(4));
    assertLongRangeSplit(
        Long.MAX_VALUE - 0x10L,
        Long.MAX_VALUE,
        4,
        true,
        Arrays.asList(
    ...
    assertLongRangeSplit(
        ...
        1,
        true,
        Arrays.asList(0x0000000000000000L, 0x0000000000000000L),
        Arrays.asList(0));
    assertLongRangeSplit(
        Long.MIN_VALUE,
        Long.MIN_VALUE,
    ...
    assertLongRangeSplit(
        ...
        4,
        true,
        Arrays.asList(0x000000000000000L, 0x000000000000000L),
        Arrays.asList(4));
    assertLongRangeSplit(
        Long.MIN_VALUE,
        Long.MIN_VALUE + 0x10L,
        4,
        true,
        Arrays.asList(
  ...
  public void testSplitLongRange() {
    ...
    assertLongRangeSplit(
        ...
        64,
        true,
        Arrays.asList(0x7fffffffffffec78L, 0x800000000000251cL),
        Arrays.asList(0));
    ...
    assertLongRangeSplit(
        Long.MIN_VALUE, Long.MAX_VALUE, 4, false, Arrays.asList(0x0L, 0xfL), Arrays.asList(60));

    // the same with precisionStep=2
    assertLongRangeSplit(
        Long.MIN_VALUE, Long.MAX_VALUE, 2, false, Arrays.asList(0x0L, 0x3L), Arrays.asList(62));
    ...
    assertLongRangeSplit(
        ...
        4,
        false,
        Arrays.asList(0x800000000000251cL, 0x800000000000251cL),
        Arrays.asList(0));
  }

  /** Note: The neededBounds Iterable must be unsigned (easier understanding what's happening) */
  private void assertIntRangeSplit(
      final int lower,
      final int upper,
      int precisionStep,
      final boolean useBitSet,
      final Iterable<Integer> expectedBounds,
      final Iterable<Integer> expectedShifts) {
    final FixedBitSet bits = useBitSet ? new FixedBitSet(upper - lower + 1) : null;
    final Iterator<Integer> neededBounds =
        (expectedBounds == null) ? null : expectedBounds.iterator();
    final Iterator<Integer> neededShifts =
        (expectedShifts == null) ? null : expectedShifts.iterator();
  ...
  public void testSplitIntRange() {
    ...
    // the same with no range splitting
    assertIntRangeSplit(
        -5000, 9500, 32, true, Arrays.asList(0x7fffec78, 0x8000251c), Arrays.asList(0));
    ...
    assertIntRangeSplit(
        ...
        8,
        false,
        Arrays.asList(0x00, 0xff),
        Arrays.asList(24));

    // the same with precisionStep=4
    assertIntRangeSplit(
        Integer.MIN_VALUE, Integer.MAX_VALUE, 4, false, Arrays.asList(0x0, 0xf), Arrays.asList(28));

    // the same with precisionStep=2
    assertIntRangeSplit(
        Integer.MIN_VALUE, Integer.MAX_VALUE, 2, false, Arrays.asList(0x0, 0x3), Arrays.asList(30));
    ...
    // a 0-length range should reproduce the range itself
    assertIntRangeSplit(
        9500, 9500, 4, false, Arrays.asList(0x8000251c, 0x8000251c), Arrays.asList(0));
  }
}
