...
public class TestJsonRequest extends SolrTestCaseHS {
  ...
  public static void doJsonRequest(Client client, boolean isDistrib) throws Exception {
    ...
    client.testJQ(
        params("json", "{query:'cat_s:A'}", "json", "{filter:'where_s:NY'}"),
        "response/numFound==1");

    // test multiple json params with one being zero length
    client.testJQ(
        params("json", "{query:'cat_s:A'}", "json", "{filter:'where_s:NY'}", "json", ""),
        "response/numFound==1");
    ...
    client.testJQ(
        params(
            "json", "{query:'*:*'}", "json", "{filter:'where_s:NY'}", "json", "{filter:'cat_s:A'}"),
        "response/numFound==1");
    ...
    client.testJQ(
        params("json.query", "'*:*'", "json.filter", "'where_s:NY'", "json.filter", "'cat_s:A'"),
        "response/numFound==1");
    ...
    client.testJQ(
        params("json", "{params:{q:'*:*', fq:['cat_s:A','where_s:NY'], start:0, rows:5, fl:id}}"),
        "response/docs==[{id:'1'}]");
    client.testJQ(
        params(
            "json",
            "{params:{q:'*:*', fq:['cat_s:A','where_s:(NY OR NJ)'], start:0, rows:1, fl:id, sort:'where_s asc'}}"),
        "response/numFound==2",
        "response/docs==[{id:'4'}]");
    client.testJQ(
        params(
            "json",
            "{params:{q:'*:*', fq:['cat_s:A','where_s:(NY OR NJ)'], start:0, rows:1, fl:[id,'x:5.5'], sort:'where_s asc'}}"),
        "response/numFound==2",
    ...
    client.testJQ(
        params(
            ...
            "{params:{q:'*:*'}}",
            "json",
            "{params:{fq:['cat_s:A','where_s:(NY OR NJ)'], start:0, rows:1, fl:[id,'x:5.5']}}",
            "json",
    ...
    client.testJQ(
        params(
            "json",
            "${OPENBRACE} query:'cat_s:A' ${CLOSEBRACE}",
            "json",
            "${OPENBRACE} filter:'where_s:NY'${CLOSEBRACE}",
            "OPENBRACE",
            "{",
            "CLOSEBRACE",
            "}"),
    ...
    // test templating with params defined in the JSON itself!  Do we want to keep this
    // functionality?
    client.testJQ(
        params(
            "json",
            "{params:{V1:A,V2:NY}, query:'cat_s:${V1}'}",
            "json",
            "{filter:'where_s:${V2}'}"),
        "response/numFound==1");
    ...
    client.testJQ(
        params(
            ...
            "{"
                ...
                + "      'should' : ["
                + "        {'#MYTAG' : 'id:1'},"
                + // tagged query (the tag should do nothing here)
                "        'id:2'"
                + "      ]"
                + "    }"
                + "  }"
    ...
    client.testJQ(
        params(
            ...
            "{ "
                ...
                + "    }"
                + "   }"
                + "   must_not : {'#NOT':{lucene : {query:'id: 1'}}}"
                + // testing tagging syntax at the same time (the tag should do nothing here)
                "  }"
                + " }"
                + "}"),
        "response/numFound==1");
    ...
    client.testJQ(
        params(
            ...
            "{"
                ...
                + "}"),
        "response/numFound==2",
        isDistrib ? "" : "response/docs==[{id:'4'},{id:'1'}]");
    ...
    client.testJQ(
        params(
            ...
            "{"
                ...
                + "}"),
        "response/numFound==2",
        isDistrib ? "" : "response/docs==[{id:'1'},{id:'4'}]");
    ...
    client.testJQ(
        params(
            ...
            "{"
                ...
                + "}"),
        "response/numFound==3",
        isDistrib ? "" : "response/docs==[{id:'4'},{id:'5'},{id:'1'}]");
    ...
    client.testJQ(
        params(
            ...
            "{"
                ...
                + "}"),
        "response/numFound==3",
        isDistrib ? "" : "response/docs==[{id:'4'},{id:'1'},{id:'5'}]");

    // TODO: this seems like a reasonable capability that we would want to support in the future.
    // It should be OK to make this pass.
    Exception e =
        expectThrows(
            Exception.class,
            () -> {
