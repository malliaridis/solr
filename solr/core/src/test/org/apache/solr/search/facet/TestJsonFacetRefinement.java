...
public class TestJsonFacetRefinement extends SolrTestCaseHS {
    ...
    for (String test : tests) {
      ...
      if (err != null) {
        throw new RuntimeException(
            "JSON failed validation. error=" + err + "\n expected =" + test + "\n got = " + json);
      }
    }
  }
  ...
  public void testMerge() throws Exception {
    ...
    doTestRefine(
        "{x : {type:terms, field:X, limit:2, refine:true} }", // the facet request
    ...
    // same test as above, but shard1 indicates it doesn't have any more results, so there shouldn't
    // be any refinement
    doTestRefine(
        "{x : {type:terms, field:X, limit:2, refine:true} }", // the facet request
        "{x: {buckets:[{val:x1, count:5}, {val:x2, count:3}],more:true } }", // shard0 response
        ...
        null, // shard0 expected refinement info
        null // shard1 expected refinement info  // without more:true, we should not attempt to get
        // extra bucket
        );
    // same but with processEmpty:true we should check for refinement even if there isn't "more"
    doTestRefine(
        "{x : {type:terms, field:X, limit:2, refine:true, facet: { processEmpty:true } } }",
        "{x: {buckets:[{val:x1, count:5}, {val:x2, count:3}],more:true } }", // shard0 response
    ...
    doTestRefine(
        "{x : {type:terms, field:X, limit:2} }", // the facet request
        "{x: {buckets:[{val:x1, count:5}, {val:x2, count:3}], more:true } }", // shard0 response
        "{x: {buckets:[{val:x2, count:4}, {val:x3, count:2}], more:true } }", // shard1 response
        null, // shard0 expected refinement info
    ...
    doTestRefine(
        "{top:{type:query, q:'foo_s:myquery', facet:{x : {type:terms, field:X, limit:2, refine:true} } } }", // the facet request
        "{top: {x: {buckets:[{val:x1, count:5}, {val:x2, count:3}], more:true } } }", // shard0
        // response
        "{top: {x: {buckets:[{val:x2, count:4}, {val:x3, count:2}], more:true } } }", // shard1
        // response
        null, // shard0 expected refinement info
    ...
    doTestRefine(
        "{top:{type:query, q:'foo_s:myquery', facet:{x : {type:terms, field:X, limit:2, refine:false} } } }",
    ...
    doTestRefine(
        "{top:{type:terms, field:Afield, facet:{x : {type:terms, field:X, limit:2, refine:true} } } }",
    ...
    doTestRefine(
        "{top:{type:range, field:R, start:0, end:1, gap:1, facet:{x : {type:terms, field:X, limit:2, refine:true} } } }",
    ...
    doTestRefine(
        "{top:{type:range, other:all, field:R, start:0, end:1, gap:1, "
    ...
    doTestRefine(
        "{top:{type:range, other:all, field:R, start:0, end:3, gap:2 } }",
        // phase #1
    ...
    doTestRefine(
        "{top:{type:range, field:R, ranges:[{from:0, to:1}], facet:{x : {type:terms, field:X, limit:2, refine:true} } } }",
    ...
    doTestRefine(
        "{top:{type:range, field:R, ranges:[{from:\"*\", to:1}], facet:{x : {type:terms, field:X, limit:2, refine:true} } } }",
    ...
    doTestRefine(
        "{top:{type:range, other:all, field:R, ranges:[{from:0, to:2},{from:2, to:3}] } }",
        // phase #1
    ...
    doTestRefine(
        "{top:{type:terms, field:Afield, refine:true, limit:1, facet:{x : {type:terms, field:X, limit:1, refine:true} } } }",
    ...
    doTestRefine(
        "{top:{type:terms, field:Afield, refine:true, limit:1, missing:true, facet:{x : {type:terms, field:X, limit:1, refine:true} } } }",
  ...
  public void testMergeWithOverrefine() throws Exception {
    ...
    doTestRefine(
        ...
        "=={x:{_l:[x1]}}");
    doTestRefine(
        "{x : {type:terms, field:X, limit:1, overrequest:0, sort:'count asc', refine:true} }",
        //
        "{x: {buckets:[{val:x1, count:3}, {val:x2, count:5}, {val:x9, count:42}, {val:x0, count:42}], more:true } }",
        "{x: {buckets:[{val:x2, count:2}, {val:x3, count:4}, {val:x7, count:66}, {val:x8, count:66}], more:true } }",
        //
    ...
    // be refined
    doTestRefine(
        "{x : {type:terms, field:X, limit:1, sort:'count asc', refine:true} }",
        //
        "{x: {buckets:[{val:x1, count:3}, {val:x2, count:5}, {val:x9, count:42}, {val:x0, count:42}], more:true } }",
        "{x: {buckets:[{val:x2, count:2}, {val:x3, count:4}, {val:x7, count:66}, {val:x8, count:66}], more:true } }",
        //
    ...
    for (String extra :
      ...
      // w/o any overrefinement, we should only need to backfill x1 & x3 (x2 already fully
      // populated)
      doTestRefine(
          "{x : {type:terms, field:X, limit:3, sort:'count desc', refine:true" + extra + " } }",
          //
    ...
    // be refined
    doTestRefine(
        "{x : {type:terms, field:X, limit:1, mincount:5, sort:'count desc', refine:true } }",
        //
    ...
    doTestRefine(
        "{x : {type:terms, field:X, limit:1, overrequest:1, overrefine:2, sort:'count asc', refine:true} }",
        //
        "{x: {buckets:[{val:x1, count:3}, {val:x2, count:5}, {val:x9, count:42}, {val:x0, count:42}], more:true } }",
    ...
    // offset=1 + limit=1 + overrefine=2 == 4 total, but x2 is fully populated, so only x1,x3,x9
    // need refined
    doTestRefine(
        "{x : {type:terms, field:X, limit:1, offset:1, overrequest:1, overrefine:2, sort:'count asc', refine:true} }",
        //
  ...
  /**
   * When <code>prelim_sort</code> is used, all 'top bucket' choices for refinement should still be
   * based on it, not the <code>sort</code> param, so this test is just some sanity checks that the
   * presence of the these params doesn't break anything in the refine / logic.
   */
  @Test
  public void testRefinementMergingWithPrelimSort() throws Exception {

    doTestRefine(
        "{x : { type:terms, field:X, limit:2, refine:true, prelim_sort:'count desc', sort:'y asc',"
            + "       facet:{ y:'sum(y_i)' } } }",
        // shard0 response
  ...
  public void testSortedFacetRefinementPushingNonRefinedBucketBackIntoTopN() throws Exception {
    ...
    client.testJQ(
        params(
            ...
            "json.facet",
            "{"
                + " cat_count:{ type:terms, field:cat_s, limit:3, overrequest:"
                + overreq
                + "             , refine:true, sort:'count asc' },"
                + " cat_price:{ type:terms, field:cat_s, limit:3, overrequest:"
                + overreq
    ...
    for (String top_refine : Arrays.asList("true", "false")) {
      ...
      client.testJQ(
          params(
              ...
              "json.facet",
              "{"
                  + " all:{ type:terms, field:all_ss, limit:1, refine:"
                  + top_refine
                  + ", overrequest:0"
                  + "       , facet:{"
                  + "   cat_count:{ type:terms, field:cat_s, limit:3, overrequest:"
      ...
      // to get past the filler terms, we should get accurate counts for (C and) Z which should push
      // X out
      client.testJQ(
          params(
              "q",
      ...
      for (boolean pe : Arrays.asList(false, true)) {
        ModifiableSolrParams p =
            params(
                ...
                "json.facet",
                "{"
                    + " top:{ type:terms, field:cat_s, limit:1, overrequest:"
                    + overrequest
                    + ", "
                    + "       refine:true, sort: 'debug asc', "
                    + "       facet:{ debug:'debug(numShards)', processEmpty:"
                    + pe
                    + " } } }");
        try {
          client.testJQ(
  ...
  @AwaitsFix(bugUrl = "https://issues.apache.org/jira/browse/SOLR-12556")
  @Test
  public void testSortedSubFacetRefinementWhenParentOnlyReturnedByOneShardProcessEmpty()
      throws Exception {
    final int numDocs = initSomeDocsWhere1ShardHasOnlyParentFacetField();
    final Client client = servers.getClient(random().nextInt());
    final List<SolrClient> clients = client.getClientProvider().all();
    ...
    client.testJQ(
        ...
        "facets=={ count: "
            ...
            + ", "
            + "      child:{ buckets:[ "
            + "                   {val:c1,count:3, debug:"
            + numClients
            + "},"
            + "                   {val:c0,count:1, debug:"
            + numClients
            + "},"
            + "      ] } },"
            + "    { val:pX, count: 13,"
            ...
            + ", "
            + "      child:{ buckets:[ "
            + "                   {val:c0,count:2, debug:"
            + numClients
            + "},"
            + "                   {val:c1,count:1, debug:"
            + numClients
            + "},"
            + "      ] } },"
            + "  ] } }");
  ...
  public void testBasicRefinement() throws Exception {
    ...
    p =
        params(
            "cat_s", "cat_s", "cat_i", "cat_i", "date", "cat_dt", "xy_s", "xy_s", "num_d", "num_d",
            "qw_s", "qw_s", "er_s", "er_s");
    doBasicRefinement(p);
    ...
    p =
        params(
            "cat_s", "cat_ss", "cat_i", "cat_is", "date", "cat_dts", "xy_s", "xy_ss", "num_d",
            "num_d", "qw_s", "qw_ss", "er_s", "er_ss");
    doBasicRefinement(p);

    // single valued docvalues
    p =
        params(
            "cat_s", "cat_sd", "cat_i", "cat_id", "date", "cat_dtd", "xy_s", "xy_sd", "num_d",
            "num_dd", "qw_s", "qw_sd", "er_s", "er_sd");
    doBasicRefinement(p);
    ...
    clients
        ...
        .add(
            sdoc(
                ...
                7,
                er_s,
                "R")); // "R" will only be picked up via refinement when parent facet is cat_s
    ...
    for (String method :
      ...
      client.testJQ(
          ...
          "facets=={ count:8"
              ...
              + // w/o overrequest and refinement, count is lower than it should be (we don't see
              // the A from the middle shard)
              "}");
      ...
      client.testJQ(
          ...
          "facets=={ count:8"
              ...
              + // w/o overrequest, we need refining to get the correct count.
              "}");
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  + "}"),
          "facets=={ count:8"
              + ", cat0:{ buckets:[ {val:'2001-01-01T01:01:01Z',count:4} ] }"
              + // w/o overrequest, we need refining to get the correct count.
              "}");
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  + "}}"
                  + "}"),
          "facets=={ count:8" + ", q1:{ count:8, cat0:{ buckets:[ {val:A,count:4} ] }   }" + "}");

      // basic refining test through/under a range facet
      for (String end :
          Arrays.asList( // all of these end+hardened options should produce the same buckets
              "end:20, hardend:true", // evenly divisible so shouldn't matter
              "end:20, hardend:false",
              "end:20", // defaults to hardened:false
              "end:5, hardend:false",
              "end:5")) {
        ...
        client.testJQ(
            params(
                ...
                "*:*",
                "json.facet",
                "{ r1 : { type:range, field:${num_d} other:all, start:-20, gap:40, "
                    + end
                    + "         , facet:{"
                    + sub
                    + "}}}"),
        ...
        client.testJQ(
            params(
                ...
                "*:*",
                "json.facet",
                "{ r1 : { type:range, field:${num_d} other:between, start:-20, gap:20, "
                    + end
                    + "         , facet:{"
                    + sub
                    + "}}}"),
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  + "}"),
          "facets=={ count:8" + ", cat0:{ buckets:[ {val:A,count:4, stat1:4.0} ] }" + "}");
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  + ",qfacet:{type:query, q:'*:*', facet:{  cat2:{${terms} type:terms, field:${cat_s}, sort:'min1 asc', limit:1, overrequest:0, refine:true,  facet:{ min1:'min(${num_d})'}   }  }}"
                  + // refinement needed through a query facet
                  ",allf:{${terms} type:terms, field:all_s,  facet:{  cat3:{${terms} type:terms, field:${cat_s}, sort:'min1 asc', limit:1, overrequest:0, refine:true,  facet:{ min1:'min(${num_d})'}   }  }}"
                  + // refinement needed through field facet
                  ",sum1:'sum(${num_d})'"
                  + // make sure that root bucket stats aren't affected by refinement
                  "}"),
          "facets=={ count:8"
              + ", cat0:{ buckets:[ {val:A,count:3, min1:-19.0} ] }"
              + // B wins in shard2, so we're missing the "A" count for that shard w/o refinement.
              ", cat1:{ buckets:[ {val:A,count:4, min1:-19.0} ] }"
              + // with refinement, we get the right count
              ", qfacet:{ count:8,  cat2:{ buckets:[ {val:A,count:4, min1:-19.0} ] }    }"
              + // just like the previous response, just nested under a query facet
              ", allf:{ buckets:[  {cat3:{ buckets:[ {val:A,count:4, min1:-19.0} ] }  ,count:8,val:all   }]  }"
              + // just like the previous response, just nested under a field facet
              ", sum1:2.0"
              + "}");
      ...
      client.testJQ(
          ...
          "facets=={ count:8"
              ...
              + // just like the previous response, just nested under a field facet
              ", cd:{ buckets:[  {val:1, count:4, xy:{buckets:[ {val:X,count:3}]}  }]  }"
              + // just like the previous response, just nested under a field facet (int type)
              ", ef:{ buckets:[  {val:'2001-01-01T01:01:01Z', count:4, xy:{buckets:[ {val:X,count:3}]}  }]  }"
              + // just like the previous response, just nested under a field facet (date type)
              "}");
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  + ",ab2:{${terms} type:terms, field:${cat_s}, limit:1, overrequest:0, refine:false, facet:{  xy:{${terms} type:terms, field:${xy_s}, limit:1, overrequest:0, refine:true}, qq:{query:'*:*'},ww:'sum(${num_d})'  }}"
                  + // top level refine=false shouldn't matter
                  ...
                  + ",allf2:{${terms} type:terms, field:all_s, limit:1, overrequest:0, refine:false, facet:{cat:{${terms} type:terms, field:${cat_s}, limit:1, overrequest:0, refine:true}, qq:{query:'*:*'},ww:'sum(${num_d})'  }}"
                  + // top level refine=false shouldn't matter
                  "}"),
          "facets=={ count:8"
              + ", ab:{ buckets:[  {val:A, count:4, xy:{buckets:[ {val:X,count:3}]}    ,qq:{count:4}, ww:4.0 }]  }"
              + // make sure qq and ww are included for _p buckets
              ", allf:{ buckets:[ {count:8, val:all, cat:{buckets:[{val:A,count:4}]} ,qq:{count:8}, ww:2.0 }]  }"
              + // make sure qq and ww are excluded (not calculated again in another phase) for _s
              // buckets
              ", ab2:{ buckets:[  {val:A, count:4, xy:{buckets:[ {val:X,count:3}]}    ,qq:{count:4}, ww:4.0 }]  }"
              + // make sure qq and ww are included for _p buckets
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  +
                  // test some values missing in sub-facet (and test that this works with normal
                  // partial bucket refinement)
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  +
                  // using overrefine only so we aren't fooled by 'local maximum' and ask all shards
                  // for 'B'
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  + "}"),
          "facets=={ count:8"
              + ", cat:{ numBuckets:2, buckets:[  {val:A, count:3, er:{numBuckets:1,buckets:[{count:2, val:E}]  }}]  }"
              + // the "R" bucket will not be seen w/o refinement
      ...
      client.testJQ(
          params(
              p,
              "q",
              "${xy_s}:Y", // query only matches one doc per shard
              "json.facet",
              "{"
                  + "  cat0:{${terms} type:terms, field:${cat_s}, "
                  + sort_limit_over
      ...
      client.testJQ(
          params(
              ...
              "{"
                  ...
                  "  all:{${terms} type:terms, field:all_s, "
                  + sort_limit_over
                  + " refine:true, "
                  + "       facet:{  "
                  +
                  // subfacet will facet on cat after joining on all (so all docs should be included
                  // in subfacet)
                  "         cat0:{${terms} type:terms, field:${cat_s}, "
                  + sort_limit_over
                  + " refine:true,"
    ...
    for (FacetField.FacetMethod m : FacetField.FacetMethod.values()) {
      client.testJQ(
          ...
          "facets=={ count:5"
              ...
              + "                   child_"
              + m
              + ":{buckets:[{val:B, count:1}]}}]}}" // * (see below)
          );
    }
