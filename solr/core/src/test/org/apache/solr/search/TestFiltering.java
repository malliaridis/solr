...
public class TestFiltering extends SolrTestCaseJ4 {
  ...
  public void testCaching() throws Exception {
    ...
    assertJQ(req("q", "*:*", "fq", "{!frange l=2 u=3 cache=false}val_i"), "/response/numFound==2");
    ...
    assertEquals(0, DelegatingCollector.setLastDelegateCount - prevCount);
    assertJQ(req("q", "*:*", "fq", "{!frange l=2 u=4 cost=0}val_i"), "/response/numFound==3");
    assertEquals(0, DelegatingCollector.setLastDelegateCount - prevCount);
    assertJQ(req("q", "*:*", "fq", "{!frange l=2 u=4 cost=999}val_i"), "/response/numFound==3");
    assertEquals(0, DelegatingCollector.setLastDelegateCount - prevCount);

    // no caching and explicitly low cost avoids post filtering
    prevCount = DelegatingCollector.setLastDelegateCount;
    assertJQ(
        req("q", "*:*", "fq", "{!frange l=2 u=5 cache=false cost=0}val_i"),
        "/response/numFound==3");
    assertEquals(0, DelegatingCollector.setLastDelegateCount - prevCount);
    ...
    assertJQ(
        req(
            ...
            "*:*",
            "fq",
            "{!frange l=2 u=6 cache=false}val_i"),
        "/response/numFound==3");
    assertEquals(1, DelegatingCollector.setLastDelegateCount - prevCount);

    // since we need the docset and the filter was not cached, the collector will need to be used
    // again
    prevCount = DelegatingCollector.setLastDelegateCount;
    assertJQ(
        req(
    ...
    prevCount = DelegatingCollector.setLastDelegateCount;
    assertJQ(
        req("facet", "true", "facet.field", "id", "q", "*:*", "fq", "{!frange l=2 u=7}val_i"),
        "/response/numFound==3");
    assertEquals(0, DelegatingCollector.setLastDelegateCount - prevCount);
    assertJQ(
        req(
    ...
    assertJQ(
        req(
            ...
            "*:*",
            "fq",
            "{!frange l=2 u=8 cache=false cost=0}val_i"),
        "/response/numFound==3");
    assertEquals(0, DelegatingCollector.setLastDelegateCount - prevCount);
  ...
  String rangeStr(
      String field, boolean negative, int l, int u, boolean cache, int cost, boolean exclude) {
    String topLev = "";
    if (!cache || exclude) {
      topLev =
          "{!"
              + (cache || random().nextBoolean() ? " cache=" + cache : "")
              + (cost != 0 ? " cost=" + cost : "")
              + ((exclude) ? " tag=t" : "")
              + "}";
    }
  ...
  String makeRandomQuery(Model model, boolean facetQuery) {
    ...
    boolean exclude =
        facetQuery ? false : random().nextBoolean(); // can't exclude a facet query from faceting

    FixedBitSet[] sets =
        facetQuery
            ? new FixedBitSet[] {model.facetQuery}
            : (exclude
                ? new FixedBitSet[] {model.answer, model.facetQuery}
                : new FixedBitSet[] {model.answer, model.multiSelect, model.facetQuery});
    ...
    if (random().nextInt(100) < 60) {
      ...
      return random().nextBoolean()
          ? frangeStr(
              f, !positive, l, u, cache, cost,
              exclude) // todo: frange doesn't work on the string field?
