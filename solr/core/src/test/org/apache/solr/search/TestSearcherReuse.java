...
/**
 * Requests to open a new searcher w/o any underlying change to the index exposed by the current
 * searcher should result in the same searcher being re-used.
 *
 * <p>Likewise, if there <em>is</em> in fact an underlying index change, we want to assert that a
 * new searcher will in fact be opened.
 */
public class TestSearcherReuse extends SolrTestCaseJ4 {
  ...
  public static void assertMainSearcher(SolrIndexSearcher s) {
    assertTrue(
        "Searcher isn't 'main': " + s.toString(),
