...
/**
 * Generates test files for testing other implementations of HLL serialization/deserialization,
 * namely the PostgreSQL implementation.
 */
public class IntegrationTestGenerator {
  ...
  // Tests
  /**
   * Cumulatively adds random values to a FULL HLL through the small range correction, uncorrected
   * range, and large range correction of the HLL's cardinality estimator.
   *
   * <p>Format: cumulative add Tests: - FULL cardinality computation
   */
  private static void fullCardinalityCorrectionTest(final ISchemaVersion schemaVersion)
      throws IOException {
  ...
  /**
   * Cumulatively unions "underpopulated" FULL HLLs into the accumulator to verify the correct
   * behavior from the PostgreSQL implementation. The PostgreSQL implementation's representations of
   * probabilistic HLLs should depend exclusively on the chosen SPARSE-to-FULL cutoff.
   *
   * <p>Format: cumulative union Tests: - EMPTY U "underpopulated" FULL => SPARSE - SPARSE U
   * "underpopulated" FULL => SPARSE - SPARSE U "barely underpopulated" FULL => FULL
   */
  private static void sparseFullRepresentationTest(final ISchemaVersion schemaVersion)
      throws IOException {
    ...
    output.write(
        stringCardinality(fullHLL)
            + ","
            + toByteA(fullHLL, schemaVersion)
            + ","
            + stringCardinality(sparseHLL)
            + ","
            + toByteA(sparseHLL, schemaVersion)
            + "\n");
    output.flush();
    ...
    output.write(
        stringCardinality(fullHLL2)
            + ","
            + toByteA(fullHLL2, schemaVersion)
            + ","
            + stringCardinality(sparseHLL)
            + ","
            + toByteA(sparseHLL, schemaVersion)
    ...
    output.write(
        stringCardinality(fullHLL3)
            + ","
            + toByteA(fullHLL3, schemaVersion)
            + ","
            + stringCardinality(sparseHLL)
            + ","
            + toByteA(sparseHLL, schemaVersion)
  ...
  /**
   * Cumulatively sets successive registers to: <code>(registerIndex % REGISTER_MAX_VALUE) + 1
   * </code> by adding specifically constructed values to a SPARSE HLL. Does not induce promotion.
   *
   * <p>Format: cumulative add Tests: - SPARSE addition (predictable)
   */
   ...
   *
   * <p>Format: cumulative add Tests: - SPARSE addition (random)
   */
  ...
  /**
   * Cumulatively sets the first register (index 0) to value 2, the last register (index m-1) to
   * value 2, and then sets registers with indices in the range 2 to (sparseCutoff + 2) to value 1
   * to trigger promotion.
   *
   * <p>This tests for register alignment in the promotion from SPARSE to FULL.
   *
   * <p>Format: cumulative add Tests: - SPARSE addition - SPARSE to FULL promotion
   */
  ...
  /**
   * Unions an EMPTY accumulator with EXPLICIT HLLs, each containing a single random value.
   *
   * <p>Format: cumulative union Tests: - EMPTY U EXPLICIT - EXPLICIT U EXPLICIT - EXPLICIT to
   * SPARSE promotion - SPARSE U EXPLICIT
   */
  private static void explicitPromotionTest(final ISchemaVersion schemaVersion) throws IOException {
    ...
    for (int i = 0;
        i < (EXPLICIT_THRESHOLD + 500) /*should be greater than promotion cutoff*/;
        i++) {
  ...
  /**
   * Unions an EMPTY accumulator with SPARSE HLLs, each having one register set.
   *
   * <p>Format: cumulative union Tests: - EMPTY U SPARSE - SPARSE U SPARSE - SPARSE promotion -
   * SPARSE U FULL
   */
  private static void sparseProbabilisticPromotionTest(final ISchemaVersion schemaVersion)
      throws IOException {
    ...
    for (int i = 0;
        i < (SPARSE_THRESHOLD + 1000) /*should be greater than promotion cutoff*/;
        i++) {
  ...
  /**
   * Unions an EMPTY accumulator with EXPLICIT HLLs, each having a single random value, twice in a
   * row to verify that the set properties are satisfied.
   *
   * <p>Format: cumulative union Tests: - EMPTY U EXPLICIT - EXPLICIT U EXPLICIT
   */
  ...
  /**
   * Unions an EMPTY accumulator with SPARSE HLLs, each having a single register set, twice in a row
   * to verify that the set properties are satisfied.
   *
   * <p>Format: cumulative union Tests: - EMPTY U SPARSE - SPARSE U SPARSE
   */
  private static void sparseProbabilisticOverlapTest(final ISchemaVersion schemaVersion)
      throws IOException {
  ...
  /**
   * Unions an EMPTY accumulator with FULL HLLs, each having many registers set, twice in a row to
   * verify that the set properties are satisfied.
   *
   * <p>Format: cumulative union Tests: - EMPTY U FULL - FULL U FULL
   */
  private static void probabilisticUnionTest(final ISchemaVersion schemaVersion)
      throws IOException {
   ...
   *
   * <p>Format: cumulative union Tests: - hopefully all union possibilities
   */
  ...
  // Helpers
  /**
   * Shortcut for testing constructor, which uses the constants defined at the top of the file as
   * default parameters.
   *
   * @return a new {@link HLL} of specified type, which uses the parameters ({@link #LOG2M}, {@link
   *     #REGWIDTH}, {@link #EXPLICIT_THRESHOLD}, and {@link #SPARSE_THRESHOLD}) specified above.
   */
  ...
  /**
   * Returns the algorithm-specific cardinality of the specified {@link HLL} as a {@link String}
   * appropriate for comparison with the algorithm-specific cardinality provided by the PostgreSQL
   * implementation.
   *
   * @param hll the HLL whose algorithm-specific cardinality is to be printed. This cannot be <code>
   *     null</code>.
   * @return the algorithm-specific cardinality of the instance as a PostgreSQL- compatible String.
   ...
   *
   * @param schemaVersion Schema version of the output. This cannot be <code>null</code>.
   * @param description Description string used to build the filename. This cannot be <code>null
   *     </code>.
   * @param type {@link TestType type} of the test file to be written. This cannot be <code>null
   *     </code>.
   * @return The opened {@link Writer writer}. This will never be <code>null</code>.
   */
  private static Writer openOutput(
      final ISchemaVersion schemaVersion, final String description, final TestType type)
      throws IOException {
   ...
   * @param schemaVersion the schema with which to serialize the HLLs. This cannot be <code>null
   *     </code>.
   */
  private static void cumulativeAddLine(
      final Writer output, final HLL hll, final long rawValue, final ISchemaVersion schemaVersion)
      throws IOException {
    hll.addRaw(rawValue);
    final String accumulatorCardinality = stringCardinality(hll);

    output.write(
        accumulatorCardinality + "," + rawValue + "," + toByteA(hll, schemaVersion) + "\n");
    output.flush();
  }
   ...
   *     </code>.
   */
  private static void initLineAdd(
      final Writer output, final HLL hll, final ISchemaVersion schemaVersion) throws IOException {
   ...
   * @param output The output {@link Writer writer}. This cannot be <code>null</code>.
   * @param hll The "accumulator" HLL instance. This cannot be <code>null</code>.
   * @param increment The "increment" HLL instance which will be unioned into the accumulator. This
   *     cannot be <code>null</code>.
   * @param schemaVersion the schema with which to serialize the HLLs. This cannot be <code>null
   *     </code>.
   */
  private static void cumulativeUnionLine(
      final Writer output, final HLL hll, final HLL increment, final ISchemaVersion schemaVersion)
      throws IOException {
    hll.union(increment);
    ...
    output.write(
        incrementCardinality
            + ","
            + toByteA(increment, schemaVersion)
            + ","
            + accumulatorCardinality
            + ","
            + toByteA(hll, schemaVersion)
            + "\n");
    output.flush();
  }
  ...
  private static enum TestType {
    /**
     * This type of test is characterized by values being added to an accumulator HLL whose
     * serialized representation (after the value is added) is printed to each line along with the
     * cardinality and added value.
     */
    ADD,
    /**
