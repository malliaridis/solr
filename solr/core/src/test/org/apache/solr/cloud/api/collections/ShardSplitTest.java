...
public class ShardSplitTest extends BasicDistributedZkTest {
  ...
  private void doSplitStaticIndexReplication(SolrIndexSplitter.SplitMethod splitMethod)
      throws Exception {
    waitForThingsToLevelOut(15, TimeUnit.SECONDS);
    ...
    try (CloudSolrClient client =
      ...
      try {
        ...
        RequestStatusState state =
            CollectionAdminRequest.requestStatus(asyncId).waitFor(client, 120);
        if (state == RequestStatusState.COMPLETED) {
          ...
          if (state == RequestStatusState.COMPLETED) {
                ...
                .registerCollectionStateWatcher(
                    ...
                    (liveNodes, collectionState) -> {
                      ...
                      if (slice.getReplicas().size() == 2) {
                        if (slice.getReplicas().stream()
                            .noneMatch(r -> r.getState() == Replica.State.RECOVERING)) {
            ...
            fail(
                "Adding a replica to sub-shard did not complete even after waiting for 30 seconds!. Saw state = "
                    + state.getKey());
          }
        } else {
          fail(
  ...
  /**
   * Used to test that we can split a shard when a previous split event left sub-shards in
   * construction or recovery state.
   *
   * <p>See SOLR-9439
   */
  @Test
  ...
  private void doSplitMixedReplicaTypes(SolrIndexSplitter.SplitMethod splitMethod)
      throws Exception {
    waitForThingsToLevelOut(15, TimeUnit.SECONDS);
  ...
  public void testSplitWithChaosMonkey() throws Exception {
    ...
    try {
      for (int i = 0; i < 1; i++) {
        StoppableIndexingThread thread =
            new StoppableIndexingThread(controlClient, cloudClient, String.valueOf(i), true);
        indexers.add(thread);
        thread.start();
      }
                ...
                if (slice != null && slice.getReplicas().size() > 1) {
                  ...
                  if (killed.compareAndSet(false, true)) {
                    ...
                    try {
                      ...
                      log.error(
                          "Monkey unable to kill jetty at port {}", cjetty.jetty.getLocalPort(), e);
                    }
                  }
                }
                log.info(
    ...
    try {
      ...
      try {
        ...
        log.warn(
            "Failed to get request status, maybe because the overseer node was shutdown by monkey",
            e);
      }
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "Shard split request state is {}",
            splitStatus == null ? "unknown" : splitStatus.getKey());
      }
      stop.set(true);
      monkeyThread.join();
      ...
      if (splitStatus == RequestStatusState.COMPLETED) {
        ...
        if (latch.getCount() != 0) {
          // sanity check
          fail(
              "We think that split was successful but sub-shard states were not updated even after 2 minutes.");
        }
      }
      ...
      assertEquals(
          "Found " + ctrlDocs + " control docs and " + cloudClientDocs + " cloud docs",
          ctrlDocs,
          cloudClientDocs);
      ...
      if (areSubShardsActive.get()) {
        ...
        assertEquals(
            "We should have checked consistency for exactly 2 replicas of shard1_1",
            2,
            numReplicasChecked);
      }
    } finally {
  ...
  public void testSplitLocking() throws Exception {
    ...
    String path =
        ZkStateReader.COLLECTIONS_ZKNODE + "/" + collectionName + "/" + SHARD1 + "-splitting";
  ...
  private void doSplitShardWithRule(SolrIndexSplitter.SplitMethod splitMethod) throws Exception {
    ...
    CollectionAdminRequest.SplitShard splitShardRequest =
        CollectionAdminRequest.splitShard(collectionName)
            .setShardName("shard1")
  ...
  private void splitByUniqueKeyTest() throws Exception {
    ...
    for (int id = 0; id <= 100; id++) {
      ...
      indexAndUpdateCount(
          router, ranges, docCounts, shardKey + "!" + String.valueOf(id), id, documentIds);
    }
    commit();
    ...
    Thread indexThread =
        new Thread(
            () -> {
              ...
              log.info(
                  "SHARDSPLITTEST: Going to add {} number of docs at 1 doc per {} ms", max, sleep);
  ...
  private void splitByRouteKeyTest() throws Exception {
    ...
    try (CloudSolrClient client = createCloudClient(null)) {
      Map<String, Object> props =
          Map.of(
              REPLICATION_FACTOR, replicationFactor, CollectionHandlingUtils.NUM_SLICES, numShards);
  ...
  protected void checkDocCountsAndShardStates(
    ...
    assertEquals(
        "Wrong number of replicas created for shard1_0",
        numReplicas,
        slice1_0.getReplicas().size());
    assertEquals(
        "Wrong number of replicas created for shard1_1",
        numReplicas,
        slice1_1.getReplicas().size());
  ...
  protected void checkSubShardConsistency(String shard) throws SolrServerException, IOException {
    ...
    for (Replica replica : slice.getReplicas()) {
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "Shard: {} Replica: {} has {} docs",
            shard,
            coreUrl,
            String.valueOf(response.getResults().getNumFound()));
      }
      assertTrue(
          "Shard: " + shard + " Replica: " + coreUrl + " has 0 docs",
  ...
  protected void indexAndUpdateCount(
      DocRouter router,
  ...
  protected void deleteAndUpdateCount(
      DocRouter router, List<DocRouter.Range> ranges, int[] docCounts, String id) throws Exception {
  ...
  public static int getHashRangeIdx(DocRouter router, List<DocRouter.Range> ranges, String id) {
    ...
    for (int i = 0; i < ranges.size(); i++) {
      ...
      if (range.includes(hash)) return i;
    }
    return -1;
  }

  protected void logDebugHelp(
      int[] docCounts,
      QueryResponse response,
      long shard10Count,
      QueryResponse response2,
    ...
    for (int i = 0; i < response2.getResults().size(); i++) {
      ...
      if (old != null) {
        log.error(
            "EXTRA: ID: {}  on shard1_1. Old version: {} new version: {}",
