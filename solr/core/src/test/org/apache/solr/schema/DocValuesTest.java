...
public class DocValuesTest extends SolrTestCaseJ4 {
      ...
      try {
        ...
        FunctionValues values =
            floatDv
                ...
                .getValues(null, searcher.getSlowAtomicReader().leaves().get(0));
  ...
  public void testDocValuesSorting() {
    ...
    assertU(commit());
    assertQ(
        req("q", "*:*", "sort", "floatdv desc", "rows", "1", "fl", "id"),
        "//str[@name='id'][.='2']");
    assertQ(
        req("q", "*:*", "sort", "intdv desc", "rows", "1", "fl", "id"), "//str[@name='id'][.='2']");
    assertQ(
        req("q", "*:*", "sort", "doubledv desc", "rows", "1", "fl", "id"),
        "//str[@name='id'][.='1']");
    assertQ(
        req("q", "*:*", "sort", "longdv desc", "rows", "1", "fl", "id"),
        "//str[@name='id'][.='1']");
    assertQ(
        req("q", "*:*", "sort", "datedv desc", "rows", "1", "fl", "id,datedv"),
        "//str[@name='id'][.='2']",
        "//result/doc[1]/date[@name='datedv'][.='1997-12-31T23:59:59.999Z']");
    assertQ(
        req("q", "*:*", "sort", "stringdv desc", "rows", "1", "fl", "id"),
        "//str[@name='id'][.='4']");
    assertQ(
        req("q", "*:*", "sort", "floatdv asc", "rows", "1", "fl", "id"),
        "//str[@name='id'][.='4']");
    assertQ(
        req("q", "*:*", "sort", "intdv asc", "rows", "1", "fl", "id"), "//str[@name='id'][.='3']");
    assertQ(
        req("q", "*:*", "sort", "doubledv asc", "rows", "1", "fl", "id"),
        "//str[@name='id'][.='3']");
    assertQ(
        req("q", "*:*", "sort", "longdv asc", "rows", "1", "fl", "id"), "//str[@name='id'][.='3']");
    assertQ(
        req("q", "*:*", "sort", "datedv asc", "rows", "1", "fl", "id"), "//str[@name='id'][.='1']");
    assertQ(
        req("q", "*:*", "sort", "stringdv asc", "rows", "1", "fl", "id"),
        "//str[@name='id'][.='2']");
    assertQ(
        req("q", "*:*", "sort", "booldv asc", "rows", "10", "fl", "booldv,stringdv"),
  ...
  public void testDocValuesFaceting() {
    ...
    assertQ(req("q", "booldv:true"), "//*[@numFound='83']");

    assertQ(req("q", "booldv:false"), "//*[@numFound='17']");
  ...
  public void testDocValuesStats() {
    ...
    assertQ(
        req("q", "*:*", "stats", "true", "rows", "0", "stats.field", "stringdv"),
        "//str[@name='min'][.='abc0']",
        "//str[@name='max'][.='abc6']",
        "//long[@name='count'][.='50']");

    assertQ(
        req("q", "*:*", "stats", "true", "rows", "0", "stats.field", "floatdv"),
        "//double[@name='min'][.='0.0']",
        "//double[@name='max'][.='1.0']",
        "//long[@name='count'][.='50']",
    ...
    assertQ(
        req("q", "*:*", "stats", "true", "rows", "0", "stats.field", "intdv"),
        "//double[@name='min'][.='0.0']",
        "//double[@name='max'][.='2.0']",
        "//long[@name='count'][.='50']",
    ...
    assertQ(
        req("q", "*:*", "stats", "true", "rows", "0", "stats.field", "doubledv"),
        "//double[@name='min'][.='0.0']",
        "//double[@name='max'][.='3.0']",
        "//long[@name='count'][.='50']",
    ...
    assertQ(
        req("q", "*:*", "stats", "true", "rows", "0", "stats.field", "longdv"),
        "//double[@name='min'][.='0.0']",
        "//double[@name='max'][.='4.0']",
        "//long[@name='count'][.='50']",
    ...
    assertQ(
        req("q", "*:*", "stats", "true", "rows", "0", "stats.field", "datedv"),
  ...
  /**
   * Tests the ability to do basic queries (without scoring, just match-only) on docvalues fields
  ...
  public void testDocValuesMatch() {
    ...
    // string: termquery
    assertQ(
        req("q", "stringdv:car", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=4]");

    // string: range query
    assertQ(
        req("q", "stringdv:[b TO d]", "sort", "id_i asc"),
        "//*[@numFound='3']",
    ...
    // string: prefix query
    assertQ(
        req("q", "stringdv:c*", "sort", "id_i asc"),
        "//*[@numFound='2']",
    ...
    assertQ(
        req("q", "stringdv:c?r", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=4]");

    // string: regexp query
    assertQ(
        req("q", "stringdv:/c[a-b]r/", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=4]");

    // float: termquery
    assertQ(
        req("q", "floatdv:3", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=3]",
        "//result/doc[2]/str[@name='id'][.=4]");
    ...
    // float: rangequery
    assertQ(
        req("q", "floatdv:[2 TO 3]", "sort", "id_i asc"),
        "//*[@numFound='3']",
        "//result/doc[1]/str[@name='id'][.=1]",
        "//result/doc[2]/str[@name='id'][.=3]",
        "//result/doc[3]/str[@name='id'][.=4]");
    ...
    // (neg) float: termquery
    assertQ(
        req("q", "floatdv:\"-5\"", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");

    // (neg) float: rangequery
    assertQ(
        req("q", "floatdv:[-6 TO -4]", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");

    // (cross zero bounds) float: rangequery
    assertQ(
        req("q", "floatdv:[-6 TO 2.1]", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=1]",
    ...
    // int: termquery
    assertQ(
        req("q", "intdv:1", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=3]");

    // int: rangequery
    assertQ(
        req("q", "intdv:[3 TO 4]", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=1]",
        "//result/doc[2]/str[@name='id'][.=2]");
    ...
    // (neg) int: rangequery
    assertQ(
        req("q", "intdv:[-1 TO 1]", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=3]",
        "//result/doc[2]/str[@name='id'][.=4]");
    ...
    // long: rangequery
    assertQ(
        req("q", "longdv:[1 TO 2]", "sort", "id_i asc"),
        "//*[@numFound='3']",
        "//result/doc[1]/str[@name='id'][.=2]",
        "//result/doc[2]/str[@name='id'][.=3]",
        "//result/doc[3]/str[@name='id'][.=4]");
    ...
    // double: termquery
    assertQ(
        req("q", "doubledv:3.1", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=1]");

    // double: rangequery
    assertQ(
        req("q", "doubledv:[2 TO 3.3]", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=1]",
        "//result/doc[2]/str[@name='id'][.=3]");
    ...
    assertQ(
        req("q", "doubledv:\"-4.3\"", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");

    // (neg) double: rangequery
    assertQ(
        req("q", "doubledv:[-6 TO -4]", "sort", "id_i asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");

    // (cross zero bounds) double: rangequery
    assertQ(
        req("q", "doubledv:[-6 TO 2.0]", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=2]",
        "//result/doc[2]/str[@name='id'][.=4]");
    ...
    assertQ(
        req("q", "booldv:false", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=1]",
        "//result/doc[2]/str[@name='id'][.=3]");

    assertQ(
        req("q", "booldv:true", "sort", "id_i asc"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.=2]",
        "//result/doc[2]/str[@name='id'][.=4]");
  }
  ...
  public void testFloatAndDoubleRangeQueryRandom() {
    ...
    Function<Supplier<Number>, Number> noNaN =
        (next) -> {
          Number num;
          while (String.valueOf(num = next.get()).equals("NaN"))
            ;
          return num;
        };
    List<Supplier<Number>> nextRandNoNaN =
        Arrays.asList(
            () -> noNaN.apply(() -> Float.intBitsToFloat(random().nextInt())),
            () -> noNaN.apply(() -> Double.longBitsToDouble(random().nextLong())));
    List<Function<Number, Long>> toSortableLong =
        Arrays.asList(
            (num) -> (long) NumericUtils.floatToSortableInt(num.floatValue()),
            (num) -> NumericUtils.doubleToSortableLong(num.doubleValue()));

    // Number minusZero[] = new Number[] {-0f, -0d}; // -0 == 0, so we should not treat them
    // differently (and we should not guarantee that sign is preserved... we should be able to index
    // both as 0)
    ...
    for (int i = 0; i < fieldName.length; i++) {
      ...
      for (int j = 0; j < numQueries; j++) {
        ...
        if (r < 5) {
          minVal = negativeInfinity[i];
          min = "*";
        } else if (r < 10) {
          minVal = specialValues[random().nextInt(specialValues.length)];
          min = String.valueOf(minVal);
        } else if (r < 15) {
          minVal = values.get(random().nextInt(values.size()));
          min = String.valueOf(minVal);
        }
        ...
        if (r < 5) {
          maxVal = positiveInfinity[i];
          max = "*";
        } else if (r < 10) {
          maxVal = specialValues[random().nextInt(specialValues.length)];
  ...
  public void testFloatAndDoubleRangeQuery() {
    ...
    String largestNegative[] =
        new String[] {
          String.valueOf(0f - Float.MIN_NORMAL), String.valueOf(0f - Double.MIN_NORMAL)
        };
    String negativeInfinity[] =
        new String[] {
          String.valueOf(Float.NEGATIVE_INFINITY), String.valueOf(Double.NEGATIVE_INFINITY)
        };
    String largestValue[] =
        new String[] {String.valueOf(Float.MAX_VALUE), String.valueOf(Double.MAX_VALUE)};
    ...
    for (int i = 0; i < fieldName.length; i++) {
      ...
      assertQ(
          req("q", fieldName[i] + ":[* TO 2]", "sort", "id_i asc", "fl", "id," + fieldName[i]),
          "//*[@numFound='4']",
          "//result/doc[1]/str[@name='id'][.=1]",
          "//result/doc[2]/str[@name='id'][.=2]",
          "//result/doc[3]/str[@name='id'][.=5]",
      ...
      assertQ(
          req("q", fieldName[i] + ":[-6 TO *]", "sort", "id_i asc", "fl", "id," + fieldName[i]),
          "//*[@numFound='6']",
          "//result/doc[1]/str[@name='id'][.=1]",
          "//result/doc[2]/str[@name='id'][.=2]",
          "//result/doc[3]/str[@name='id'][.=3]",
