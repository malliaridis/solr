...
public class PreAnalyzedFieldTest extends SolrTestCaseJ4 {
  ...
  private String addTwoDocs(int firstId, String field) {
    return "<add>\n"
        + doc(
            ...
            field,
            json(
                "{'v':'1','str':'document one','tokens':[{'t':'one'},{'t':'two'},{'t':'three','i':100}]}"))
        + doc(
            "id",
            Integer.toString(firstId + 1),
            field,
            json(
                "{'v':'1','str':'document two','tokens':[{'t':'eleven'},{'t':'twelve'},{'t':'thirteen','i':110}]}"))
        + "</add>\n";
  }
  ...
  public void testIndexAndQueryNoSchemaAnalyzer() {
    ...
    assertQ(
        req("q", "{!field f='pre_no_analyzer'}{'v':'1','tokens':[{'t':'two'}]}"),
        "//result[@numFound='1']");
    assertQ(
        req("q", "{!field f='pre_no_analyzer'}{'v':'1','tokens':[{'t':'eleven'},{'t':'twelve'}]}"),
        "//result[@numFound='1']");
  }
  ...
  // "1 =test ąćęłńóśźż \u0001=one,i=22,s=123,e=128,p=deadbeef,y=word two,i=1,s=5,e=8,y=word
