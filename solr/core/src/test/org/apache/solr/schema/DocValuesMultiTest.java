...
public class DocValuesMultiTest extends SolrTestCaseJ4 {
  ...
  public static void beforeTests() throws Exception {
    ...
    for (String f :
      ...
      assertTrue(
          f + " is not multiValued, test is useless, who changed the schema?", sf.multiValued());
  ...
  /**
   * Tests the ability to do basic queries (without scoring, just match-only) on string docvalues
  ...
  public void testStringDocValuesMatch() {
    ...
    // string: termquery
    assertQ(
        req("q", "stringdv:car", "sort", "id asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=4]");

    // string: range query
    assertQ(
        req("q", "stringdv:[b TO d]", "sort", "id asc"),
        "//*[@numFound='4']",
    ...
    assertQ(
        req("q", "stringdv:c?r", "sort", "id asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=4]");

    // string: regexp query
    assertQ(
        req("q", "stringdv:/c[a-b]r/", "sort", "id asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=4]");
  }

  /**
   * Tests the ability to do basic queries (without scoring, just match-only) on boolean docvalues
   * fields that are not inverted (indexed "forward" only)
   */
  @Test
  public void testBoolDocValuesMatch() {
    ...
    // boolean: range query,
    assertQ(
        req("q", "booldv:[false TO false]", "sort", "id asc"),
        "//*[@numFound='3']",
    ...
    assertQ(
        req("q", "*:*", "sort", "id asc", "rows", "10", "fl", "booldv"),
  ...
  public void testFloatDocValuesMatch() {
    ...
    // float: rangequery
    assertQ(
        req("q", "floatdv:[-1 TO 2.5]", "sort", "id asc"),
        "//*[@numFound='3']",
        "//result/doc[1]/str[@name='id'][.=1]",
        "//result/doc[2]/str[@name='id'][.=3]",
        "//result/doc[3]/str[@name='id'][.=5]");
    ...
    assertQ(
        req("q", "floatdv:[-6 TO -4]", "sort", "id asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");

    // (neg) float: termquery
    assertQ(
        req("q", "floatdv:\"-5\"", "sort", "id asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");
  }

  /**
   * Tests the ability to do basic queries (without scoring, just match-only) on double docvalues
   * fields that are not inverted (indexed "forward" only)
   */
  @Test
  public void testDoubleDocValuesMatch() {
    ...
    // double: rangequery
    assertQ(
        req("q", "doubledv:[-1 TO 2.5]", "sort", "id asc"),
        "//*[@numFound='3']",
        "//result/doc[1]/str[@name='id'][.=1]",
        "//result/doc[2]/str[@name='id'][.=3]",
        "//result/doc[3]/str[@name='id'][.=5]");
    ...
    assertQ(
        req("q", "doubledv:[-6 TO -4]", "sort", "id asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");

    // (neg) double: termquery
    assertQ(
        req("q", "doubledv:\"-5\"", "sort", "id asc"),
        "//*[@numFound='1']",
        "//result/doc[1]/str[@name='id'][.=2]");
  }
  ...
  public void testDocValuesFacetingSimple() {
    // this is the random test verbatim from DocValuesTest, so it populates with the default values
