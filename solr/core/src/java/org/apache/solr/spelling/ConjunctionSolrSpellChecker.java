 ...
 * {@link WordBreakSolrSpellChecker} in conjunction with a "standard" spell checker (such as {@link
...
public class ConjunctionSolrSpellChecker extends SolrSpellChecker {
  ...
  public String init(NamedList<?> config, SolrCore core) {
    for (SolrSpellChecker checker : checkers) {
      String dn = checker.init(config, core);
      ...
      if (dictionaryName != null && !dictionaryName.equals(dn)) {
        throw new IllegalArgumentException(
            "Cannot have more than one dictionary. (" + dn + " , " + dictionaryName + ")");
      }
      dictionaryName = dn;
    }
  ...
  public SpellingResult mergeSuggestions(
    ...
    SpellingResult[] results = new SpellingResult[checkers.size()];
    for (int i = 0; i < checkers.size(); i++) {
      results[i] = checkers.get(i).mergeSuggestions(mergeData, numSug, count, extendedResults);
    }
