...
public class AnalyzingLookupFactory extends LookupFactory {
  ...
  /**
   * If <code>true</code>, then a separator between tokens is preserved. This means that suggestions
  ...
  /**
   * When multiple suggestions collide to the same analyzed form, this is the limit of how many
   * unique surface forms we keep.
   */
  public static final String MAX_SURFACE_FORMS = "maxSurfaceFormsPerAnalyzedForm";

  /**
   * When building the FST ("index-time"), we add each path through the tokenstream graph as an
   * individual entry. This places an upper-bound on how many expansions will be added for a single
   * suggestion.
   */
