...
/**
 * Simple class to help with parsing a string. <b>Note: This API is experimental and may change in
...
public class StrParser {
  ...
  /**
   * Skips leading whitespace and returns whatever sequence of non whitespace it can find (or hte
   * empty string)
   */
  ...
  /**
   * Sort direction or null if current position does not indicate a sort direction. (True is desc,
   * False is asc). Position is advanced to after the comma (or end) when result is non null
   */
