...
interface SweepCountAware {
   ...
   *
   * <p>For each iterator position (each doc), {@link #registerCounts(SegCounter)} must be called
   * before this method.
   */
  boolean collectBase();
   ...
   *
   * <p>For each iterator position, this method must be called before {@link #collectBase()}
   *
   ...
   *
   * <p>First, the driver passes this object as the param to {@link
  ...
  static class SegCountPerSeg implements SegCounter {
    ...
    public void register(CountSlotAcc[] countAccs, LongValues toGlobal, int maxSegOrd) {
      ...
      // domains supplies a <code>null</code> DocIdSetIterator -- matches no docs -- for a given
