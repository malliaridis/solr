...
public class AuditEvent {
  ...
  public AuditEvent(
    ...
    this.collections =
        authorizationContext.getCollectionRequests().stream()
  ...
  /**
   * Identifies the resource being operated on. This is not the same as URL path. For queries the
   * resource is relative to collection name, e.g. /select or /update. For other events the resource
   * may be /api/node/health or /admin/collection
   */
  ...
  private RequestType findRequestType() {
    ...
    if (SEARCH_PATH_PATTERNS.stream().anyMatch(p -> p.matcher(resource).matches()))
      return RequestType.SEARCH;
    if (INDEXING_PATH_PATTERNS.stream().anyMatch(p -> p.matcher(resource).matches()))
      return RequestType.UPDATE;
    if (STREAMING_PATH_PATTERNS.stream().anyMatch(p -> p.matcher(resource).matches()))
      return RequestType.STREAMING;
    if (ADMIN_PATH_PATTERNS.stream().anyMatch(p -> p.matcher(resource).matches()))
  ...
  private static final List<String> ADMIN_PATH_REGEXES =
      Arrays.asList(
          "^/admin/.*",
          "^/api/(c|collections)$",
