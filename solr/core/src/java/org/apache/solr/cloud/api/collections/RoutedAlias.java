...
public abstract class RoutedAlias {
   ...
   */
  public static RoutedAlias fromProps(String aliasName, Map<String, String> props)
    ...
    if (routerTypes.size() == 1) {
      ...
      for (int i = 0; i < routerTypes.size(); i++) {
        ...
        dimensions.add(
            DimensionalRoutedAlias.dimensionForType(
                selectForIndex(i, props), routerType, i, () -> dra[0]));
      }
  ...
  private static Map<String, String> selectForIndex(int i, Map<String, String> original) {
    return original.entrySet().stream()
        ...
        .map(
            e ->
                new SimpleEntry<>(
                    e.getKey().replaceAll("(.*\\.)" + i + "\\.(.*)", "$1$2"), e.getValue()))
        .collect(Collectors.toMap(SimpleEntry::getKey, SimpleEntry::getValue));
  }

  private static RoutedAlias routedAliasForType(
   ...
   *
   * <p>Routed Aliases do not aggregate existing collections, instead they create collections on the
   * fly. If the initial collection can be determined from initialization parameters it should be
   * calculated here.
   *
  ...
  /**
   * The name of the alias. This name is used in place of a collection name for both queries and
   * updates.
   *
  ...
  static class CandidateCollection {
    ...
    CandidateCollection(
        CreationType creationType, String destinationCollection, String creationCollection) {
  ...
  /**
   * Create as many collections as required. This method loops to allow for the possibility that the
   * route value requires more than one collection to be created. Since multiple threads may be
   * invoking maintain on separate requests to the same alias, we must pass in a descriptor that
   * details what collection is to be created. This assumption is checked when the command is
   * executed in the overseer. When this method finds that all collections required have been
   * created it returns the (possibly new) destination collection for the document that caused the
   * creation cycle.
   *
  ...
  private String createAllRequiredCollections(
      AddUpdateCommand cmd, CandidateCollection targetCollectionDesc) {
