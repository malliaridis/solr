 ...
 *
 * <p>The <code>DocExpirationUpdateProcessorFactory</code> provides two features related to the
 * "expiration" of documents which can be used individually, or in combination:
 *
 * <ol>
 *   <li>Computing expiration field values for documents from a "time to live" (TTL)
 ...
 * that expiration was explicitly set with-out leveraging the TTL feature of this factory.
 *
 * <p>The following configuration options are supported:
 *
 * <ul>
 *   <li><code>expirationFieldName</code> - The name of the expiration field to use in any
 *       operations (mandatory).
 *   <li><code>ttlFieldName</code> - Name of a field this process should look for in each document
 *       processed, defaulting to <code>_ttl_</code>. If the specified field name exists in a
 *       document, the document field value will be parsed as a {@linkplain DateMathParser Date Math
 *       Expression} relative to <code>NOW</code> and the result will be added to the document using
 *       the <code>expirationFieldName</code>. Use <code>&lt;null name="ttlFieldName"/&gt;</code> to
 *       disable this feature.
 *   <li><code>ttlParamName</code> - Name of an update request param this process should look for in
 *       each request when processing document additions, defaulting to <code>_ttl_</code>. If the
 *       specified param name exists in an update request, the param value will be parsed as a
 *       {@linkplain DateMathParser Date Math Expression} relative to <code>NOW</code> and the
 *       result will be used as a default for any document included in that request that does not
 *       already have a value in the field specified by <code>ttlFieldName</code>. Use <code>
 *       &lt;null name="ttlParamName"/&gt;</code> to disable this feature.
 *   <li><code>autoDeletePeriodSeconds</code> - Optional numeric value indicating how often this
 *       factory should trigger a delete to remove documents. If this option is used, and specifies
 *       a non-negative numeric value, a background thread will be created that will execute
 *       recurring <code>deleteByQuery</code> commands using the specified period. The delete query
 *       will remove all documents with an <code>expirationFieldName</code> up to <code>NOW</code>.
 *   <li><code>autoDeleteChainName</code> - Optional name of an <code>updateRequestProcessorChain
 *       </code> to use when executing automatic deletes. If not specified, or <code>&lt;null/&gt;
 *       </code>, the default <code>updateRequestProcessorChain</code> for this collection is used.
 *       This option is ignored unless <code>autoDeletePeriodSeconds</code> is configured and is
 *       non-negative.
 * </ul>
 *
 * <p>For example: The configuration below will cause any document with a field named <code>_ttl_
 * </code> to have a Date field named <code>_expire_at_</code> computed for it when added -- but no
 ...
 * &lt;/processor&gt;</pre>
 *
 * <p>Alternatively, in this configuration deletes will occur automatically against the <code>
 * _expire_at_</code> field every 5 minutes - but this processor will not automatically populate the
 * <code>_expire_at_</code> using any sort of TTL expression. Only documents that were added with an
 * explicit <code>_expire_at_</code> field value will ever be deleted.
 *
 * <pre class="prettyprint">
 * &lt;processor class="solr.processor.DocExpirationUpdateProcessorFactory"&gt;
 ...
 * &lt;/processor&gt;</pre>
 *
 * <p>This last example shows the combination of both features using a custom <code>ttlFieldName
 * </code>: Documents with a <code>my_ttl</code> field will have an <code>_expire_at_</code> field
 * computed, and deletes will be triggered every 5 minutes to remove documents whose <code>
 * _expire_at_</code> field value is in the past.
 *
 * <pre class="prettyprint">
 * &lt;processor class="solr.processor.DocExpirationUpdateProcessorFactory"&gt;
 *   &lt;int name="autoDeletePeriodSeconds"&gt;300&lt;/int&gt;
...
public final class DocExpirationUpdateProcessorFactory extends UpdateRequestProcessorFactory
  ...
  private String removeArgStr(
      final NamedList<?> args, final String arg, final String def, final String errMsg) {
  ...
  public void init(NamedList<?> args) {
    ...
    deleteChainName =
        removeArgStr(
            args, DEL_CHAIN_NAME_CONF, null, "must be a <str> or <null/> for default chain");

    ttlField =
        removeArgStr(
            args, TTL_FIELD_NAME_CONF, DEF_TTL_KEY, "must be a <str> or <null/> to disable");
    ttlParam =
        removeArgStr(
            args, TTL_PARAM_NAME_CONF, DEF_TTL_KEY, "must be a <str> or <null/> to disable");
  ...
  private static final class TTLUpdateProcessor extends UpdateRequestProcessor {
    ...
    public void processAdd(AddUpdateCommand cmd) throws IOException {
      ...
      final String math =
          doc.containsKey(ttlField) ? doc.getFieldValue(ttlField).toString() : defaultTtl;
  ...
  /**
   * Runnable that uses the <code>deleteChainName</code> configured for this factory to execute a
   * delete by query (using the configured <code>expireField</code>) followed by a soft commit to
   * re-open searchers (if needed)
   *
   * <p>This logic is all wrapped up in a new SolrRequestInfo context with some logging to help make
   * it obvious this background activity is happening.
   *
   * <p>In cloud mode, this runner only triggers deletes if {@link #iAmInChargeOfPeriodicDeletes} is
   * true. (logging is minimal in this situation)
   *
    ...
    public void run() {
      ...
      try (LocalSolrQueryRequest req =
        ...
        try {
          ...
          if (null == proc) {
            log.warn(
                "No active processors, skipping automatic deletion of expired docs using chain: {}",
                deleteChainName);
            return;
          }
          try {
   ...
   *
   * <p>If this method returns false, it may have also logged a message letting the user know why we
   * aren't attempting period deletion (but it will attempt to not log this excessively)
   */
  private boolean iAmInChargeOfPeriodicDeletes() {
    ...
    if (previouslyInChargeOfDeletes && !inChargeOfDeletesRightNow) {
      ...
      log.info(
          "Not currently in charge of periodic deletes for this collection, {}",
