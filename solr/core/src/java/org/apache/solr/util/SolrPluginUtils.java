...
public class SolrPluginUtils {
  ...
  /**
   * Map containing all the possible purposes codes of a request as key and the corresponding
   * readable purpose as value
   */
   ...
   *
   * <p>RequestHandlers can use this method to ensure their defaults and overrides are visible to
   * other components such as the response writer
   *
  ...
  public static void setDefaults(
      SolrQueryRequest req, SolrParams defaults, SolrParams appends, SolrParams invariants) {
  ...
  private static SolrParams applyParamSet(
  ...
  /**
   * SolrIndexSearch.numDocs(Query,Query) freaks out if the filtering query is null, so we use this
   * workarround.
   */
   ...
   *
   * <p>This is an entirely optional step which you might want to perform for the following reasons:
   *
   * <ul>
   *   <li>Locates the document-retrieval costs in one spot, which helps detailed performance
   *       measurement
   *   <li>Determines a priori what fields will be needed to be fetched by various subtasks, like
   *       response writing and highlighting. This minimizes the chance that many needed fields will
   *       be loaded lazily. (it is more efficient to load all the field we require normally).
   * </ul>
   *
   * If lazy field loading is disabled, this method does nothing.
   */
  public static void optimizePreFetchDocs(
  ...
  public static NamedList<NamedList<Object>> explanationsToNamedLists(
    ...
    NamedList<NamedList<Object>> out = new SimpleOrderedMap<>();
  ...
  /**
   * Given a string containing fieldNames and boost info, converts it to a Map from field name to
   * boost info.
   *
   * <p>Doesn't care if boost info is negative, you're on your own.
   *
   * <p>Doesn't care if boost info is missing, again: you're on your own.
   *
   ...
   * (which may be null).
   *
   * @param fieldLists an array of Strings eg. <code>{"fieldOne^2.3", "fieldTwo", fieldThree^-0.4}
   *     </code>
   * @return Map of fieldOne =&gt; 2.3, fieldTwo =&gt; null, fieldThree =&gt; -0.4
   */
   ...
   *
   * @param fieldLists - an array of Strings eg. <code>
   *     {"fieldOne^2.3", "fieldTwo", fieldThree~5^-0.4}</code>
   ...
   */
  public static List<FieldParams> parseFieldBoostsAndSlop(
      String[] fieldLists, int wordGrams, int defaultSlop) {
    if (null == fieldLists || 0 == fieldLists.length) {
    ...
    for (String in : fieldLists) {
      ...
      for (String s : fieldConfigs) {
        ...
        float boost =
            (1 == fieldAndSlopVsBoost.length) ? 1 : Float.parseFloat(fieldAndSlopVsBoost[1]);
  ...
  /**
   * Checks the number of optional clauses in the query, and compares it with the specification
   * string to determine the proper value to use.
   *
   * <p>If mmAutoRelax=true, we'll perform auto relaxation of mm if tokens are removed from some but
   * not all DisMax clauses, as can happen when stopwords or punctuation tokens are removed in
   * analysis.
   *
   * <p>Details about the specification format can be found <a
   * href="doc-files/min-should-match.html">here</a>
   *
   * <p>A few important notes...
   *
   * <ul>
   *   <li>If the calculations based on the specification determine that no optional clauses are
   *       needed, BooleanQuerysetMinMumberShouldMatch will never be called, but the usual rules
   *       about BooleanQueries still apply at search time (a BooleanQuery containing no required
   *       clauses must still match at least one optional clause)
   *   <li>
   *   <li>No matter what number the calculation arrives at, BooleanQuery.setMinShouldMatch() will
   *       never be called with a value greater then the number of optional clauses (or less then 1)
   * </ul>
   *
   * <p>:TODO: should optimize the case where number is same as clauses to just make them all
   * "required"
   *
   * @param q The query as a BooleanQuery.Builder
   * @param spec The mm spec
   * @param mmAutoRelax whether to perform auto relaxation of mm if tokens are removed from some but
   *     not all DisMax clauses
   */
  ...
  static int calculateMinShouldMatch(int optionalClauseCount, String spec) {
    ...
    if (-1 < spec.indexOf('%')) {
      ...
      int percent = checkedParseInt(spec, "Invalid 'mm' spec. Expecting an integer.");
  ...
  private static void flattenBooleanQuery(
      BooleanQuery.Builder to, BooleanQuery from, float fromBoost) {
  ...
  /**
   * Returns its input if there is an even (ie: balanced) number of '"' characters -- otherwise
   * returns a String in which all '"' characters are striped out.
   */
   ...
   * destArr} at the position of the document in the response. destArr is assumed to be the same
   * size as {@code resultIds} is. {@code resultIds} comes from {@link ResponseBuilder#resultIds}.
   * If the doc key isn't in {@code resultIds} then it is ignored. Note: most likely you will call
   * {@link #removeNulls(Map.Entry[], NamedList)} sometime after calling this.
   */
  public static void copyNamedListIntoArrayByDocPosInResponse(
  ...
  public static class DisjunctionMaxQueryParser extends SolrQueryParser {
     ...
     * @param fieldBoosts the mapping from fieldname to boost value that should be used to build up
     *     the clauses of the DisjunctionMaxQuery.
     ...
     * DisjunctionMaxQuery. (so yes: aliases which point at other aliases should work)
     */
    @Override
  ...
  public static void invokeSetters(
      Object bean, Iterable<? extends Map.Entry<String, ?>> initArgs, boolean lenient) {
    if (initArgs == null) return;
    final Class<?> clazz = bean.getClass();
    for (Map.Entry<String, ?> entry : initArgs) {
      String key = entry.getKey();
      String setterName =
          "set" + String.valueOf(Character.toUpperCase(key.charAt(0))) + key.substring(1);
      try {
        ...
        throw new RuntimeException(
            "Error invoking setter "
                + setterName
                + " on class : "
                + clazz.getName()
                + ". This might be a case of SOLR-12207",
            ae);
      }
    }
  }
  ...
  private static Method findSetter(
      Class<?> clazz, String setterName, String key, Class<?> paramClazz, boolean lenient) {
    BeanInfo beanInfo;
    try {
   ...
   * present that's not known this method will return {@value #UNKNOWN_VALUE}
   *
