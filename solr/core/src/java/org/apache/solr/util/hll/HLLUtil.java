...
final class HLLUtil {
  ...
  /**
   * Precomputed <code>twoToL</code> values indexed by a linear combination of <code>regWidth</code>
   * and <code>log2m</code>.
   *
   * <p>The array is one-dimensional and can be accessed by using index <code>
   * (REG_WIDTH_INDEX_MULTIPLIER * regWidth) + log2m</code> for <code>regWidth</code> and <code>
  ...
  static {
    for (int regWidth = HLL.MINIMUM_REGWIDTH_PARAM;
        regWidth <= HLL.MAXIMUM_REGWIDTH_PARAM;
        regWidth++) {
  ...
  // ************************************************************************
  /**
   * Computes the bit-width of HLL registers necessary to estimate a set of the specified
   * cardinality.
   *
   * @param expectedUniqueElements an upper bound on the number of unique elements that are
   ...
   *
   * @param m this must be a power of two, cannot be less than 16 (2<sup>4</sup>), and cannot be
   *     greater than 65536 (2<sup>16</sup>).
   * @return gamma times <code>registerCount</code> squared where gamma is based on the value of
   *     <code>registerCount</code>.
  ...
  /**
   * The "small range correction" formula from the HyperLogLog algorithm. Only appropriate if both
   * the estimator is smaller than
   *
   * <pre>(5/2) * m</pre>
   *
   ...
   *
   * @param m the number of registers in the HLL. <em>m<em> in the paper.
   * @param numberOfZeroes the number of registers with value zero. <em>V</em> in the paper.
