...
/**
 * Models a single (value, count) pair that will exist in the collection of values for a {@link
 * PivotFacetField} parent. This <code>PivotFacetValue</code> may itself have a nested {@link
 * PivotFacetField} child
 *
 * @see PivotFacetField
...
public class PivotFacetValue {
  ...
  // named list with objects because depending on how big the counts are we may get either a long or
  // an int
  ...
  /**
   * The {@link PivotFacetField} corrisponding to the nested child pivot for this <code>
   * PivotFacetValue</code>. May be null if this object is the leaf of a pivot.
   */
  ...
  /**
   * A <b>NON-Recursive</b> method indicating if the specified shard has already contributed to the
  ...
  /**
   * A recursive method for generating a NamedList from this value suitable for including in a pivot
   * facet response to the original distributed request.
   *
