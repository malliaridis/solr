 ...
 * withs pecial tracking of a {@link PivotFacetValue} corrisponding to the <code>null</code> value
...
public class PivotFacetFieldValueCollection implements Iterable<PivotFacetValue> {
   ...
   *
   * <p>Any caller that manipulates the {@link PivotFacetValue}s contained in this collection must
  ...
  /**
   * The {@link PivotFacetValue} with corresponding to a value of <code>null</code> when {@link
   * FacetParams#FACET_MISSING} is used.
   *
   * @return the appropriate <code>PivotFacetValue</code> object, may be null if we "missing" is not
   ...
   *
   * @param value of the <code>PivotFacetValue</code> to lookup, if <code>null</code> this returns
   *     the same as {@link #getMissingValue}
   * @return the corrisponding <code>PivotFacetValue</code> or null if there is no <code>
   *     PivotFacetValue</code> in this collection corrisponding to the specified value.
   */
   ...
   * the <code>PivotFacetValue</code> corrisponding to the missing-value.
   *
   * @see #getExplicitValuesList
   * @see List#get(int)
   * @see #getMissingValue
   ...
   *
   * <p>This method should only be called after all refinement is completed.
   *
    ...
    Iterator<PivotFacetValue> it =
        new Iterator<PivotFacetValue>() {
          ...
          public PivotFacetValue next() {
            ...
            } else if (shouldGiveMissingValue) {
  ...
  /**
   * A helper method for use in <code>Comparator</code> classes where object properties are <code>
   * Comparable</code> but may be null.
   */
