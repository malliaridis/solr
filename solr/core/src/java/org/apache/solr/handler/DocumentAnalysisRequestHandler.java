 ...
 * handler expects a (single) content stream of the following format: <br>
 *
 * <pre><code>
 *  &lt;docs&gt;
 *      &lt;doc&gt;
 ...
 *
 * <br>
 * <em><b>Note: Each document must contain a field which serves as the unique key. This key is used
 * in the returned response to associate an analysis breakdown to the analyzed document.</b></em>
 *
 * <p>Like the {@link org.apache.solr.handler.FieldAnalysisRequestHandler}, this handler also
...
public class DocumentAnalysisRequestHandler extends AnalysisRequestHandlerBase {
  ...
  public void init(NamedList<?> args) {
    ...
    try {
      ...
      log.debug(
          "Unable to set the 'reuse-instance' property for the input factory: {}", inputFactory);
    }
  }
   ...
   */
  DocumentAnalysisRequest resolveAnalysisRequest(SolrQueryRequest req)
  ...
  /**
   * Handles the resolved {@link DocumentAnalysisRequest} and returns the analysis response as a
   * named list.
   *
  ...
  NamedList<Object> handleAnalysisRequest(DocumentAnalysisRequest request, IndexSchema schema) {
    ...
    for (SolrInputDocument document : request.getDocuments()) {
      ...
      for (String name : document.getFieldNames()) {
        ...
        try {
          termsToMatch =
              (queryValue != null && request.isShowMatch())
        ...
        if (request.getQuery() != null) {
          try {
            AnalysisContext analysisContext =
                new AnalysisContext(fieldType, fieldType.getQueryAnalyzer(), EMPTY_BYTES_SET);
        ...
        for (Object fieldValue : fieldValues) {
          indexTokens.add(
              String.valueOf(fieldValue), analyzeValue(fieldValue.toString(), analysisContext));
        }
   ...
   * @param schema The index schema. The schema is used to validate that the read document has a
   *     unique key field.
  ...
  /**
   * Extracts the only content stream from the request. {@link
   * org.apache.solr.common.SolrException.ErrorCode#BAD_REQUEST} error is thrown if the request
   * doesn't hold any content stream or holds more than one.
   *
   * @param req The solr request.
