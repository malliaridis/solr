 ...
 *
 * <p>You create a new LegacyNumericRangeQuery with the static factory methods, eg:
 *
 ...
 *
 * <p>The performance of LegacyNumericRangeQuery is much better than the corresponding {@link
 ...
 *
 * <p>You can optionally specify a <a href="#precisionStepDesc"><code>precisionStep</code></a> when
 ...
 * between <b>1</b> and <b>8</b>. A good starting point to test is <b>4</b>, which is the default
 * value for all <code>Numeric*</code> classes. See <a href="#precisionStepDesc">below</a> for
 * details.
 *
 * <p>This query defaults to {@linkplain MultiTermQuery#CONSTANT_SCORE_REWRITE}. With precision
 * steps of &le;4, this query can be run with one of the BooleanQuery rewrite methods without
 * changing BooleanQuery's default max clause count. <br>
 *
 * <h2>How it works</h2>
 *
 * <p>See the publication about <a target="_blank" href="http://www.panfmp.org">panFMP</a>, where
 * this algorithm was described (referred to as <code>TrieRangeQuery</code>):
 *
 * <blockquote>
 *
 * <strong>Schindler, U, Diepenbroek, M</strong>, 2008. <em>Generic XML-based Framework for Metadata
 * Portals.</em> Computers &amp; Geosciences 34 (12), 1947-1955. <a
 * href="http://dx.doi.org/10.1016/j.cageo.2008.02.023"
 * target="_blank">doi:10.1016/j.cageo.2008.02.023</a>
 *
 * </blockquote>
 *
 * <p><em>A quote from this paper:</em> Because Apache Lucene is a full-text search engine and not a
 ...
 * searched only with the lowest possible precision in the <em>trie</em>, while the boundaries are
 * matched more exactly. This reduces the number of terms dramatically.
 *
 * <p>For the variant that stores long values in 8 different precisions (each reduced by 8 bits)
 * that uses a lowest precision of 1 byte, the index contains only a maximum of 256 distinct values
 * in the lowest precision. Overall, a range could consist of a theoretical maximum of <code>
 * 7*255*2 + 255 = 3825</code> distinct terms (when there is a term for every distinct value of an
 ...
 *
 * <p>You can choose any <code>precisionStep</code> when encoding values. Lower step values mean
 ...
 *
 * <p style="font-family:serif">&nbsp;&nbsp;indexedTermsPerValue = <b>ceil</b><span
 * style="font-size: larger">(</span>bitsPerValue / precisionStep<span style="font-size:
 * larger">)</span> As the lower precision terms are shared by many values, the additional terms
 * only slightly grow the term dictionary (approx. 7% for <code>precisionStep=4</code>), but have a
 * larger impact on the postings (the postings file will have more entries, as every document is
 * linked to <code>indexedTermsPerValue</code> terms instead of one). The formula to estimate the
 ...
 * &nbsp;&nbsp;<img src="doc-files/nrq-formula-1.png" alt="\mathrm{termDictOverhead} =
 * \sum\limits_{i=0}^{\mathrm{indexedTermsPerValue}-1} \frac{1}{2^{\mathrm{precisionStep}\cdot i}}">
 *
 * <p>On the other hand, if the <code>precisionStep</code> is smaller, the maximum number of terms
 ...
 * <p>
 * <!-- the formula in the alt attribute was transformed from latex to PNG with http://1.618034.com/latex.php (with 110 dpi): -->
 * &nbsp;&nbsp;<img src="doc-files/nrq-formula-2.png" alt="\mathrm{maxQueryTerms} = \left[ \left(
 ...
 *
 * <p>For longs stored using a precision step of 4, <code>maxQueryTerms = 15*15*2 + 15 = 465</code>,
 * and for a precision step of 2, <code>maxQueryTerms = 31*3*2 + 3 = 189</code>. But the faster
 * search speed is reduced by more seeking in the term enum of the index. Because of this, the ideal
 * <code>precisionStep</code> value can only be found out by testing. <b>Important:</b> You can
 ...
 *
 * <ul>
 *   <li>The default for all data types is <b>4</b>, which is used, when no <code>precisionStep
 *       </code> is given.
 *   <li>Ideal value in most cases for <em>64 bit</em> data types <em>(long, double)</em> is
 *       <b>6</b> or <b>8</b>.
 *   <li>Ideal value in most cases for <em>32 bit</em> data types <em>(int, float)</em> is <b>4</b>.
 *   <li>For low cardinality fields larger precision steps are good. If the cardinality is &lt; 100,
 *       it is fair to use {@link Integer#MAX_VALUE} (see below).
 *   <li>Steps <b>&ge;64</b> for <em>long/double</em> and <b>&ge;32</b> for <em>int/float</em>
 ...
 *       this case simply use {@link Integer#MAX_VALUE} as <code>precisionStep</code>). Using {@link
 ...
 *       fields using one of the above <code>precisionSteps</code>.
 * </ul>
 *
...
public final class LegacyNumericRangeQuery<T extends Number> extends MultiTermQuery {
   ...
   * value to <code>null</code>. By setting inclusive to false, it will match all documents
   ...
   * ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting the min or max value to
   * <code>null</code>. By setting inclusive to false, it will match all documents excluding the
   ...
   * range using the given <a href="#precisionStepDesc"><code>precisionStep</code></a>. You can have
   * half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting the min or max
   * value to <code>null</code>. {@link Double#NaN} will never match a half-open range, to hit
   ...
   * too.
   */
  public static LegacyNumericRangeQuery<Double> newDoubleRange(
      final String field,
      final int precisionStep,
      Double min,
      Double max,
   ...
   * org.apache.solr.legacy.LegacyNumericUtils#PRECISION_STEP_DEFAULT} (16). You can have half-open
   * ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting the min or max value to
   * <code>null</code>. {@link Double#NaN} will never match a half-open range, to hit {@code NaN}
   ...
   * range using the given <a href="#precisionStepDesc"><code>precisionStep</code></a>. You can have
   * half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting the min or max
   * value to <code>null</code>. {@link Float#NaN} will never match a half-open range, to hit {@code
   * NaN} use a query with {@code min == max == Float.NaN}. By setting inclusive to false, it will
   * match all documents excluding the bounds, with inclusive on, the boundaries are hits, too.
   */
  public static LegacyNumericRangeQuery<Float> newFloatRange(
      final String field,
      final int precisionStep,
      Float min,
      Float max,
   ...
   *
   * <p>WARNING: This term enumeration is not guaranteed to be always ordered by {@link
