...
public class RestManager {
   ...
   *
   * <p>Registering of managed resources can happen before the RestManager is fully initialized. To
  ...
  private static class RestManagerManagedResource extends ManagedResource {
    ...
    protected synchronized void reloadFromStorage() throws SolrException {
      ...
      try {
        ...
        throw new SolrException(
            ErrorCode.SERVER_ERROR,
            "Failed to load stored data for " + resourceId + " due to: " + ioExc,
            ioExc);
      }
      ...
      if (managedData != null) onManagedDataLoadedFromStorage(managedInitArgs, managedData);
    }

    /**
     * Loads and initializes any ManagedResources that have been created but are not associated with
     * any Solr components.
     */
     ...
     *
     * <p>Called during PUT/POST processing to apply updates to the managed data passed from the
     * client.
     */
    @SuppressWarnings("unchecked")
    @Override
    ...
    public void doGet(BaseSolrResource endpoint, String childId) {
      ...
      if (!resourceIdMatcher.matches()) {
        ...
        throw new SolrException(
            ErrorCode.BAD_REQUEST, "Requests to path " + path + " not supported!");
      }
      ...
      for (ManagedResourceRegistration reg : restManager.registry.getRegistered()) {
        if (!reg.resourceId.startsWith(filter)) continue; // doesn't match filter
  ...
  /**
   * Initializes the RestManager with the storageIO being optionally created outside of this
   * implementation such as to use ZooKeeper instead of the local FS.
   */
  public void init(SolrResourceLoader loader, NamedList<String> initArgs, StorageIO storageIO)
      throws SolrException {
  ...
  /**
   * If not already registered, registers the given {@link ManagedResource} subclass at the given
   * resourceId, creates an instance. Returns the corresponding instance.
   */
  public synchronized ManagedResource addManagedResource(
      String resourceId, Class<? extends ManagedResource> clazz) {
  ...
  protected ManagedResource createManagedResource(ManagedResourceRegistration reg)
      throws SolrException {
  ...
  /**
   * Returns the {@link ManagedResource} subclass instance corresponding to the given resourceId
   * from the registry.
   *
   * @throws SolrException if no managed resource is registered with the given resourceId.
   */
  public ManagedResource getManagedResource(String resourceId) {
    ManagedResource res = getManagedResourceOrNull(resourceId);
    if (res == null) {
      throw new SolrException(
          ErrorCode.NOT_FOUND, "No ManagedResource registered for path: " + resourceId);
    }
    return res;
  }
  ...
  /**
   * Returns the {@link ManagedResource} subclass instance corresponding to the given resourceId
   * from the registry, or null if no resource has been registered with the given resourceId.
   */
